...
const aiDiagnosisCompletedResultSql = (column: string) =>
  `COALESCE((${aiDiagnosisFingerprintSql(column)} AND json_type(${column}, '$._providerRequestId') = 'text' AND trim(json_extract(${column}, '$._providerRequestId')) <> '' AND json_extract(${column}, '$._providerRequestId') = trim(json_extract(${column}, '$._providerRequestId')) AND length(json_extract(${column}, '$._providerRequestId')) <= ${AI_PROVIDER_REQUEST_ID_LIMIT} AND json_type(${column}, '$._providerModel') = 'text' AND trim(json_extract(${column}, '$._providerModel')) <> '' AND json_extract(${column}, '$._providerModel') = trim(json_extract(${column}, '$._providerModel')) AND length(json_extract(${column}, '$._providerModel')) <= ${AI_DIAGNOSIS_RUN_FIELD_LIMITS.model} AND json_type(${column}, '$.companyOverview') = 'text' AND trim(json_extract(${column}, '$.companyOverview')) <> '' AND json_type(${column}, '$.confirmedStrengths') = 'array' AND json_type(${column}, '$.mainRisks') = 'array' AND json_type(${column}, '$.solutionCandidates') = 'array' AND json_type(${column}, '$.verificationQuestions') = 'array' AND json_type(${column}, '$.missingDocuments') = 'array' AND json_type(${column}, '$.complianceNotes') = 'array' AND json_type(${column}, '$.nextAction') = 'text' AND trim(json_extract(${column}, '$.nextAction')) <> ''), 0)`;
