...
- CI의 타입 검사, 린트, 787개 Node 검사, 격리 workerd 검사, 로그인 E2E, 접근성 검사, 빌드, 번들 한도가 모두 통과한다.
...
4. 정확한 `main` 커밋으로 Sites 소스와 배포 아카이브를 만든다.
5. Sites 저장 버전의 `commit_sha`와 아카이브 커밋을 대조한다.
6. 기존 공개 범위는 바꾸지 않고 저장된 버전만 배포한다.
7. 배포 완료 뒤 익명 헬스체크와 읽기 전용 브라우저 점검을 실행한다.
...
- GitHub `main`과 서명된 릴리스 태그가 소스 기준본이다.
