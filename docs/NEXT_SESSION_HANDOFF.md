...
**최신 진행:** [운영 설정 마무리 진행](PRODUCTION_SETUP_PROGRESS_2026_09_11.md)을 먼저 따른다. 새 사용자 요청에 따라 원격 관리자 계정 생성과 공식 주소 로그인 검증을 완료했다. 회원가입 최소 길이는 6자이며, 원격 최초 관리자 14자·로컬 관리자와 복구 15자다. 관리자 재생성이나 자격증명 덮어쓰기를 하지 않는다. 기존 운영자료 복원은 아직 대기다. 아래의 관리자 0명·생성 미실행 기록은 과거 상태다.
