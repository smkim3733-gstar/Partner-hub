...
- GitHub: `https://github.com/smkim3733-gstar/Partner-hub`, 원격 `main`은 `ee40a91b5de4ef8fed06245ba2b636e623773b0c`; 로컬 버전 343 푸시는 명시 승인을 기다려 보류
...
- 운영 상태: 서버 오류 로그 개인정보 보완본 버전 107이 공개 운영 중이다. 버전 108–342는 더 완전한 로컬 후보인 버전 343으로 대체한다.
...
Sites 최신 저장 버전은 309이고 공개 운영본은 버전 107이다. 버전 108–342는 버전 343으로 대체해 배포하지 않는다. GitHub 반영에는 정확히 `버전 343 GitHub main 푸시 승인`, 버전 343의 Sites 소스 전송·버전 저장·공개 교체에는 정확히 `버전 343 Sites 소스 전송 및 공개 운영 배포 승인`이 필요하다. 버전 343을 기능 마감으로 확정해 v344 이상 신규 감사 버전은 만들지 않는다. 형식상 정상인 다른 `actor` 문자열의 공동 변조는 정상 생성 경로의 서버 전이 검증·D1 트리거가 보호하지만 파일 재고 SQL이 권위 표시명과 독립 대조하지 않는 수용 잔여 위험으로 기록한다. 실제 파트너 계정·고객 데이터·외부 발송·유료 AI는 별도 승인 없이 사용하지 않는다.
