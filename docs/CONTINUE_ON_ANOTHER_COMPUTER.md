...
- 최신 감사 커밋: `cad8de7717b11b821d268d87ba5de6d5bebcb697` (`test: preserve FLOW multipart profile continuity`)
- 최신 확인: 기존 부분 R2 예약 재시도 본문 처리 중 대표가 파트너 표시명을 바꾸어도 ChatGPT 안정 사용자 ID·회원 ID 결속과 두 번째 신원·담당·업로드 권한 검사를 유지
- 연속 확인: 정확한 재시도가 기존 명령 ID·파일 ID·R2 키로 완료되며 FLOW 불변 담당자·명령 영수증·감사기록 행위자 표시는 기존 상담 이력을 보존
...
- 현재 공개 운영본: 버전 107
- 최신 Sites 저장 버전: 278 (`appgprj_6a92514801988191b79eb9bd314e3fcd~appgver_452dbbb77ba48191a34c273d159f0a91`), 소스 `cad8de7717b11b821d268d87ba5de6d5bebcb697`
- 운영 상태: 서버 오류 로그 개인정보 보완본 버전 107이 공개 운영 중이다. 버전 108–277은 더 완전한 후보인 버전 278로 대체한다.
...
먼저 [상담 FLOW multipart 재시도 표시명 변경 경쟁 무결성](FLOW_RETRY_MULTIPART_DISPLAY_NAME_RACE_INTEGRITY_2026_09_07.md)을 확인한다.
...
Sites 버전 278 저장·검증까지 완료했고 공개 운영본은 버전 107이다. 버전 108–277은 버전 278로 대체해 배포하지 않는다. 공개 교체는 정확한 `버전 278 운영 배포 승인`이 필요하다. 30분 간격 자동 개발은 활성 상태로 부분 R2 재시도의 첫 R2 쓰기 중 파트너 계정이 정지될 때 나머지 객체 쓰기 뒤 최종 인증 검사가 D1 커밋을 차단하고 예약·객체를 복구 가능하게 보존하는지 감사한다. 실제 파트너 계정·고객 데이터·외부 발송·유료 AI는 별도 승인 없이 사용하지 않는다.
