...
- 최신 감사 커밋: `a5200ceb137d44366c65aef124232a05e2d4fe9e` (`test: cover first FLOW storage failure`)
- 최신 확인: 두 슬롯 예약 뒤 첫 R2 쓰기가 객체 생성 전에 실패하면 객체 0개·예약 2개를 유지하고 새 명령 ID의 동일 첨부 재시도가 기존 두 파일 ID·키로 완료
- 검증: Node 회귀 검사 720개, 격리 workerd/D1/R2 검사 522개, 타입검사, 전체 lint, 변경 파일 포맷, 프로덕션 빌드와 로컬 Worker 화면·인증 경계 통과
...
- 현재 공개 운영본: 버전 107
- 최신 Sites 저장 버전: 263 (`appgprj_6a92514801988191b79eb9bd314e3fcd~appgver_181a1c08b06c8191a1d5df2a04746c02`), 소스 `a5200ceb137d44366c65aef124232a05e2d4fe9e`
- 운영 상태: 서버 오류 로그 개인정보 보완본 버전 107이 공개 운영 중이다. 버전 108–262는 더 완전한 후보인 버전 263으로 대체한다.
...
먼저 [상담 FLOW 첫 R2 쓰기 실패 복구 무결성](FLOW_FIRST_R2_WRITE_FAILURE_RECOVERY_INTEGRITY_2026_09_06.md)을 확인한다.
...
Sites 버전 263 저장·검증까지 완료했고 공개 운영본은 버전 107이다. 버전 108–262는 버전 263으로 대체해 배포하지 않는다. 공개 교체는 정확한 `버전 263 운영 배포 승인`이 필요하다. 30분 간격 자동 개발은 활성 상태로 2슬롯 부분 R2 실패 뒤 원래 명령 ID 재시도의 예약·키 수렴 감사를 이어간다. 실제 파트너 계정·고객 데이터·외부 발송·유료 AI는 별도 승인 없이 사용하지 않는다.
