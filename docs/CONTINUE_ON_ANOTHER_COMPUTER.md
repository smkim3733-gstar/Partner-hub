...
- 최신 감사 커밋: `e97eafa528e46726450e9c9db12da0cb0e5b772b` (`test: cover dual FLOW storage ambiguity`)
- 최신 확인: 전사문·음성 두 R2 저장은 성공했지만 두 번째 응답만 유실된 상태에서 두 객체와 예약을 보존하고 새 명령 ID의 동일 첨부 재시도가 기존 두 파일 ID·키로 완료
- 검증: Node 회귀 검사 719개, 격리 workerd/D1/R2 검사 522개, 타입검사, 전체 lint, 변경 파일 포맷, 프로덕션 빌드와 로컬 Worker 화면·인증 경계 통과
...
- 현재 공개 운영본: 버전 107
- 최신 Sites 저장 버전: 262 (`appgprj_6a92514801988191b79eb9bd314e3fcd~appgver_dab2e6ca4204819190632eb1775b63f0`), 소스 `e97eafa528e46726450e9c9db12da0cb0e5b772b`
- 운영 상태: 서버 오류 로그 개인정보 보완본 버전 107이 공개 운영 중이다. 버전 108–261은 더 완전한 후보인 버전 262로 대체한다.
...
먼저 [상담 FLOW 2슬롯 R2 결과 불명 복구 무결성](FLOW_DUAL_AMBIGUOUS_R2_WRITE_RECOVERY_INTEGRITY_2026_09_06.md)을 확인한다.
...
Sites 버전 262 저장·검증까지 완료했고 공개 운영본은 버전 107이다. 버전 108–261은 버전 262로 대체해 배포하지 않는다. 공개 교체는 정확한 `버전 262 운영 배포 승인`이 필요하다. 30분 간격 자동 개발은 활성 상태로 2슬롯 첫 R2 쓰기가 객체 생성 전에 실패하는 예약·재시도 복구 감사를 이어간다. 실제 파트너 계정·고객 데이터·외부 발송·유료 AI는 별도 승인 없이 사용하지 않는다.
