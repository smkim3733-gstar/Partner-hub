...
- 최신 기능 커밋: `4e688479b16ef71b55c2e834b5fd3e34120b1fdb` (`fix: preserve linked company originals`)
- 최신 기능: 포털 기업자료 카드가 참조하는 원본의 직접·경합 삭제를 차단하고 D1 메타데이터·담당·진행 연결과 R2 바이트 보존
- 검증: Node 회귀 검사 558개, 격리 workerd/D1/R2 검사 347개, 타입검사, 전체 lint, 프로덕션 빌드와 로컬 운영 Worker 화면 3곳 HTTP 200·CSP 통과
...
- 최신 Sites 저장 버전: 139 (`appgprj_6a92514801988191b79eb9bd314e3fcd~appgver_4a1b910ee1f4819182315c5ba8b6c606`), 소스 `4e688479b16ef71b55c2e834b5fd3e34120b1fdb`
- 운영 상태: 서버 오류 로그 개인정보 보완본 버전 107이 공개 운영 중이다. 버전 108~138은 버전 139로 대체해 배포하지 않으며, 기업자료 연결 원본 삭제 무결성 보완본 버전 139가 정확한 버전 운영 배포 승인 대기 중이다.
...
Sites 버전 139 저장·검증까지 완료했고 공개 운영본은 버전 107이다. 버전 108~138은 버전 139로 대체해 배포하지 않는다. 버전 139 공개 배포에는 정확히 `버전 139 운영 배포 승인`이라는 사용자 명시 승인이 필요하다. 30분 간격 자동 개발은 활성 상태로 다음 고우선순위 감사를 이어간다. 다음 감사는 원본 다운로드가 D1 원장과 R2 실제 객체의 크기·삭제 상태를 응답 직전에 대조해 손상되거나 교체된 크기의 바이트를 제공하지 않는지 확인한다. 본문 해시를 새로 저장하거나 기존 원본을 자동 수정·삭제하지 않는다. 관리자 안정 ID가 실제로 바뀌는 경우의 결속 초기화는 계정 소유권 확인·운영 D1 백업·감사가 필요한 수동 복구이며 자동화하지 않는다. 실제 파트너 계정·고객 데이터·외부 발송·유료 AI·보관 및 삭제 정책은 별도 승인 없이 사용하거나 변경하지 않는다.
