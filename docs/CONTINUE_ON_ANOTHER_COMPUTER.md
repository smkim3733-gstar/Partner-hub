...
- 최신 감사 커밋: `4b8159576741fae0d1e092f97e8a0aad1366e897` (`test: recheck FLOW ownCases retry`)
- 최신 확인: 파트너 부분 R2 저장 뒤 실제 관리자 경로로 `ownCases` 권한을 회수하면 안정 계정 결속은 유지되지만 재시도는 R2 전에 403으로 차단
- 복구 확인: 권한을 복구한 정확한 재시도만 같은 명령 ID·파일 ID·R2 키를 이용해 완료
...
- 현재 공개 운영본: 버전 107
- 최신 Sites 저장 버전: 272 (`appgprj_6a92514801988191b79eb9bd314e3fcd~appgver_0ccea32f52308191bae58aaa070d4fc6`), 소스 `4b8159576741fae0d1e092f97e8a0aad1366e897`
- 운영 상태: 서버 오류 로그 개인정보 보완본 버전 107이 공개 운영 중이다. 버전 108–271은 더 완전한 후보인 버전 272로 대체한다.
...
먼저 [상담 FLOW 재시도 담당 진행 권한 회수 무결성](FLOW_RETRY_OWN_CASES_REVOCATION_INTEGRITY_2026_09_07.md)을 확인한다.
...
Sites 버전 272 저장·검증까지 완료했고 공개 운영본은 버전 107이다. 버전 108–271은 버전 272로 대체해 배포하지 않는다. 공개 교체는 정확한 `버전 272 운영 배포 승인`이 필요하다. 30분 간격 자동 개발은 활성 상태로 부분 재시도 요청의 multipart 본문을 읽는 동안 `ownCases` 권한이 회수되는 경쟁에서도 예약 생성·조회와 R2 쓰기 전에 차단되는지 감사한다. 실제 파트너 계정·고객 데이터·외부 발송·유료 AI는 별도 승인 없이 사용하지 않는다.
