...
- 최신 기능 커밋: `258f58f043b79d37b38f540fea6439bd6ce26b86` (`fix: enforce FLOW file size limits`)
- 최신 기능: FLOW 파일 목적 허용 목록, 업로드·저장 공용 5MB·8MB·25MB 상한, SQLite 축소 전 손상 목적·크기 격리
- 검증: Node 회귀 검사 590개, 격리 workerd/D1/R2 검사 384개, 타입검사, 전체 lint, 변경 파일 포맷 검사, 프로덕션 빌드와 로컬 운영 Worker 화면 3곳 HTTP 200·CSP·`DENY`·`nosniff`·`no-referrer` 통과
...
- 현재 공개 운영본: 버전 107
- 최신 Sites 저장 버전: 155 (`appgprj_6a92514801988191b79eb9bd314e3fcd~appgver_1cabef5779688191aed37ded966c3846`), 소스 `258f58f043b79d37b38f540fea6439bd6ce26b86`
- 운영 상태: 서버 오류 로그 개인정보 보완본 버전 107이 공개 운영 중이다. 버전 108~154는 버전 155로 대체해 배포하지 않으며, FLOW 파일 목적·크기 보완본 버전 155가 정확한 버전 운영 배포 승인 대기 중이다.
...
먼저 [상담 FLOW 파일 목적·크기 무결성 경계](FLOW_FILE_METADATA_SIZE_INTEGRITY_2026_09_05.md)를 확인한다.
...
Sites 버전 155 저장·검증까지 완료했고 공개 운영본은 버전 107이다. 버전 108~154는 버전 155로 대체해 배포하지 않는다. 버전 155 공개 배포에는 정확히 `버전 155 운영 배포 승인`이라는 사용자 명시 승인이 필요하다. 30분 간격 자동 개발은 활성 상태로 다음 고우선순위 감사를 이어간다. 다음 감사는 FLOW 저장 파일의 이름 확장자·고정 MIME·목적 조합이 실제 업로드 정책과 일치하는지 확인하고 상세·SQLite 대시보드 경계를 공용화한다. 관리자 안정 ID가 실제로 바뀌는 경우의 결속 초기화는 계정 소유권 확인·운영 D1 백업·감사가 필요한 수동 복구이며 자동화하지 않는다. 실제 파트너 계정·고객 데이터·외부 발송·유료 AI·보관 및 삭제 정책은 별도 승인 없이 사용하거나 변경하지 않는다.
