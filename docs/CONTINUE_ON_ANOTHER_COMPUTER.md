...
- 최신 감사 커밋: `5ec96d17e2db9942d25ce7da97f55e3a67249557` (`test: guard FLOW upload commit against case deletion`)
- 최신 확인: 기존 부분 R2 예약의 정확한 재시도가 두 객체와 최종 접근 검사를 마친 뒤 D1 FLOW 쓰기 직전에 실제 관리자 상태 저장 API로 사건 원본 삭제
- 연속 확인: 포털 상태 payload 조건부 비교의 삭제 전 FLOW·영수증·파일 원장 커밋 차단과 D1 전체 롤백, 사건 삭제 유지, 객체·`pending` 예약 보존, 사건 원본 복구 뒤 같은 파일 ID·R2 키로 한 번 완료
- 검증: Node 회귀 검사 742개, 격리 workerd/D1/R2 검사 522개, 타입검사, 전체 lint, 변경 파일 포맷, 프로덕션 빌드와 로컬 Worker 화면·인증 경계 통과
...
- 현재 공개 운영본: 버전 107
- 최신 Sites 저장 버전: 292 (`appgprj_6a92514801988191b79eb9bd314e3fcd~appgver_246157cc66e48191823681313ec004f8`), 소스 `5ec96d17e2db9942d25ce7da97f55e3a67249557`
- 운영 상태: 서버 오류 로그 개인정보 보완본 버전 107이 공개 운영 중이다. 버전 108–291은 더 완전한 후보인 버전 292로 대체한다.
...
먼저 [상담 FLOW D1 쓰기 직전 사건 삭제 경쟁 무결성](FLOW_RETRY_D1_WRITE_CASE_DELETION_RACE_INTEGRITY_2026_09_07.md)을 확인한다.
