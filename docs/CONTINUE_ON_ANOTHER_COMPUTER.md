...
- 최신 기능 커밋: `33692c8388b208506712fb6139da3225634fe364` (`fix: bind FLOW uploads to exact slots`)
- 최신 기능: 상담 녹취의 전사문·보조 음성 예약을 실제 성공 명령이 만든 `fileId`·`audioFileId`에 결속하고 교차 기기 2슬롯 재시도와 슬롯 불일치를 방어
- 검증: Node 회귀 검사 714개, 격리 workerd/D1/R2 검사 522개, 타입검사, 전체 lint, 변경 파일 포맷, 프로덕션 빌드와 로컬 Worker 화면·인증 경계 통과
...
- 현재 공개 운영본: 버전 107
- 최신 Sites 저장 버전: 257 (`appgprj_6a92514801988191b79eb9bd314e3fcd~appgver_dc173c708b6081919b9f20ddca29d0d4`), 소스 `33692c8388b208506712fb6139da3225634fe364`
- 운영 상태: 서버 오류 로그 개인정보 보완본 버전 107이 공개 운영 중이다. Sites 안전 게이트가 정확한 공개 대상 버전 승인을 요구하며, 버전 108–256은 버전 257로 대체해 배포하지 않는다.
...
먼저 [상담 FLOW 파일·음성 슬롯 결속 무결성](FLOW_UPLOAD_SLOT_BINDING_INTEGRITY_2026_09_06.md)을 확인한다.
...
Sites 버전 257 저장·검증까지 완료했고 공개 운영본은 버전 107이다. 버전 108–256은 버전 257로 대체해 배포하지 않는다. 공개 교체는 정확한 `버전 257 운영 배포 승인`이 필요하다. 30분 간격 자동 개발은 활성 상태로 두 슬롯 중 두 번째 R2 쓰기 실패 때 첫 객체 정리와 동일 재시도의 전체 복구 경계 감사를 이어간다. 실제 파트너 계정·고객 데이터·외부 발송·유료 AI는 별도 승인 없이 사용하지 않는다.
