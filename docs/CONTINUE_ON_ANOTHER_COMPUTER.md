...
- 최신 기능 커밋: `07332cd8a9652c9c3cff0f529f47e7df4289a5be` (`fix: recheck FLOW access after file inspection`)
- 최신 확인: 기존 부분 R2 예약의 정확한 multipart 재시도가 본문 뒤 접근 재검사를 마친 다음 파일 실제 형식·서명을 검사하는 동안 실제 관리자 경로로 사건과 담당 계정을 연속 삭제
- 연속 확인: 수정 전 삭제 뒤 R2 쓰기 2회 재현, 파일 검사 직후 접근 재검사로 수정 후 새 R2·예약·FLOW D1 쓰기 0건, 삭제·결속 폐기와 기존 전사문 객체·두 `pending` 예약 보존, 사건·회원 복구 뒤 재결속해 한 번 완료
- 검증: Node 회귀 검사 750개, 격리 workerd/D1/R2 검사 522개, 타입검사, 전체 lint, 변경 파일 포맷, 프로덕션 빌드와 로컬 Worker 화면·인증 경계 통과
...
- 현재 공개 운영본: 버전 107
- 최신 Sites 저장 버전: 298 (`appgprj_6a92514801988191b79eb9bd314e3fcd~appgver_be9f1614633081918bfa18139e001abe`), 소스 `07332cd8a9652c9c3cff0f529f47e7df4289a5be`
- 운영 상태: 서버 오류 로그 개인정보 보완본 버전 107이 공개 운영 중이다. 버전 108–297은 더 완전한 후보인 버전 298로 대체한다.
...
먼저 [상담 FLOW 파일 서명 검사 중 접근변경 경쟁 무결성](FLOW_CONTENT_CHECK_ACCESS_RACE_2026_09_07.md)을 확인한다.
...
Sites 버전 298 저장·검증까지 완료했고 공개 운영본은 버전 107이다. 버전 108–297은 버전 298로 대체해 배포하지 않는다. 공개 교체는 정확한 `버전 298 운영 배포 승인`이 필요하다. 30분 간격 자동 개발은 활성 상태로 파일 검사 후 재검사와 첫 R2 쓰기 사이의 업로드 예약 확인·생성 중 권한·사건 상태가 바뀔 때, R2 쓰기 전 재검사가 예약·객체·FLOW를 안전하게 보존하는지 감사한다. 실제 파트너 계정·고객 데이터·외부 발송·유료 AI는 별도 승인 없이 사용하지 않는다.
