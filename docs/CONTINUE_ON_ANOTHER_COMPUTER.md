...
- 최신 기능 커밋: `585774dd1f003c333e485e84f534be1d6a3de1c5` (`fix: recover stale AI diagnosis locks`)
- 최신 기능: Step 0 외부 요청 85초 제한과 5분 이상 지난 생성 잠금의 보존형 회수
...
- 현재 공개 운영본: 버전 107
- 최신 Sites 저장 버전: 190 (`appgprj_6a92514801988191b79eb9bd314e3fcd~appgver_1aa6fb65cd78819180991d62f97f01f0`), 소스 `585774dd1f003c333e485e84f534be1d6a3de1c5`
- 운영 상태: 서버 오류 로그 개인정보 보완본 버전 107이 공개 운영 중이다. 버전 108–189는 버전 190으로 대체해 배포하지 않으며, AI 중단 잠금 회수 보완본 버전 190이 정확한 버전 운영 배포 승인 대기 중이다.
...
먼저 [AI 중단 잠금 회수 무결성 경계](AI_STALE_LOCK_RECOVERY_INTEGRITY_2026_09_06.md)를 확인한다.
...
Sites 버전 190 저장·검증까지 완료했고 공개 운영본은 버전 107이다. 버전 108–189는 버전 190으로 대체해 배포하지 않는다. 버전 190 공개 배포에는 정확히 `버전 190 운영 배포 승인`이라는 사용자 명시 승인이 필요하다. 30분 간격 자동 개발은 활성 상태로 다음 고우선순위 감사를 이어간다. 다음 감사는 Anthropic 공급자 요청 식별값이 파서·오류 처리·완료 원장에 비용 및 장애 추적 증거로 일관되게 보존되는지 확인한다. 관리자 안정 ID가 실제로 바뀌는 경우의 결속 초기화는 계정 소유권 확인·운영 D1 백업·감사가 필요한 수동 복구이며 자동화하지 않는다. 실제 파트너 계정·고객 데이터·외부 발송·유료 AI·보관 및 삭제 정책은 별도 승인 없이 사용하거나 변경하지 않는다.
