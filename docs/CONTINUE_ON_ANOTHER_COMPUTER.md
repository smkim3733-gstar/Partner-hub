...
- 최신 감사 커밋: `04c8dba881b1512a17ae9a48dfc9dc6b4d139aca` (`test: cover ambiguous FLOW storage result`)
- 최신 확인: R2 저장 성공 뒤 응답만 유실된 결과 불명 상태에서 객체와 예약을 보존하고, 새 명령 ID의 동일 첨부 재시도가 기존 파일 ID·키로 정확히 완료
- 검증: Node 회귀 검사 718개, 격리 workerd/D1/R2 검사 522개, 타입검사, 전체 lint, 변경 파일 포맷, 프로덕션 빌드와 로컬 Worker 화면·인증 경계 통과
...
- 현재 공개 운영본: 버전 107
- 최신 Sites 저장 버전: 261 (`appgprj_6a92514801988191b79eb9bd314e3fcd~appgver_7abe5d8beeac8191b721a5fdd8f48f70`), 소스 `04c8dba881b1512a17ae9a48dfc9dc6b4d139aca`
- 운영 상태: 서버 오류 로그 개인정보 보완본 버전 107이 공개 운영 중이다. 버전 108–260은 더 완전한 후보인 버전 261로 대체한다.
...
먼저 [상담 FLOW R2 쓰기 결과 불명 복구 무결성](FLOW_AMBIGUOUS_R2_WRITE_RECOVERY_INTEGRITY_2026_09_06.md)을 확인한다.
...
Sites 버전 261 저장·검증까지 완료했고 공개 운영본은 버전 107이다. 버전 108–260은 버전 261로 대체해 배포하지 않는다. 공개 교체는 정확한 `버전 261 운영 배포 승인`이 필요하다. 30분 간격 자동 개발은 활성 상태로 2슬롯 모두 저장됐지만 두 번째 R2 성공 응답이 유실되는 결과 불명 복구 감사를 이어간다. 실제 파트너 계정·고객 데이터·외부 발송·유료 AI는 별도 승인 없이 사용하지 않는다.
