...
- 최신 기능 커밋: `c0f64bc2c8d5591aab385d4c1091201025f03645` (`fix: guard company file deletion cleanup`)
- 최신 기능: R2 삭제 뒤 D1 원본 행·키·객체 무결성·담당·진행 원장을 결정 당시 저장 키가 유지될 때만 정리하고 경합 증거 보존
- 검증: Node 회귀 검사 612개, 격리 workerd/D1/R2 검사 425개, 타입검사, 전체 lint, 변경 파일 포맷 검사, 프로덕션 빌드와 로컬 운영 Worker 화면 3곳 HTTP 200·CSP·`DENY`·`nosniff`·`no-referrer` 통과
...
- 현재 공개 운영본: 버전 107
- 최신 Sites 저장 버전: 168 (`appgprj_6a92514801988191b79eb9bd314e3fcd~appgver_65af936055a48191a2f1fe2c024a0cb5`), 소스 `c0f64bc2c8d5591aab385d4c1091201025f03645`
- 운영 상태: 서버 오류 로그 개인정보 보완본 버전 107이 공개 운영 중이다. 버전 108~167은 버전 168로 대체해 배포하지 않으며, 기업자료 삭제 후 D1 정리 무결성 보완본 버전 168이 정확한 버전 운영 배포 승인 대기 중이다.
...
먼저 [기업자료 삭제 후 D1 정리 무결성 경계](COMPANY_FILE_DELETION_CLEANUP_INTEGRITY_2026_09_05.md)를 확인한다.
...
Sites 버전 168 저장·검증까지 완료했고 공개 운영본은 버전 107이다. 버전 108~167은 버전 168로 대체해 배포하지 않는다. 버전 168 공개 배포에는 정확히 `버전 168 운영 배포 승인`이라는 사용자 명시 승인이 필요하다. 30분 간격 자동 개발은 활성 상태로 다음 고우선순위 감사를 이어간다. 다음 감사는 기업 원본의 파일명·기업·종류·제목·담당 표시·업로더·MIME·크기·생성시각을 가산 원장에 결속해 유효값끼리 변조도 권한·소비 경로 전에 차단하는지 확인한다. 관리자 안정 ID가 실제로 바뀌는 경우의 결속 초기화는 계정 소유권 확인·운영 D1 백업·감사가 필요한 수동 복구이며 자동화하지 않는다. 실제 파트너 계정·고객 데이터·외부 발송·유료 AI·보관 및 삭제 정책은 별도 승인 없이 사용하거나 변경하지 않는다.
