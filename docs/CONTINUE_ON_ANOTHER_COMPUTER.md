...
- 최신 기능 커밋: `cfde7e16c9c99d6c65176752155018179504dae1` (`fix: validate FLOW payload structure`)
- 최신 기능: 상담 FLOW payload 필수 문자열·배열·객체를 서버 저장 읽기·SQLite 축소 투영·클라이언트 응답에서 공통 검증
- 검증: Node 회귀 검사 570개, 격리 workerd/D1/R2 검사 361개, 타입검사, 전체 lint, 프로덕션 빌드와 로컬 운영 Worker 화면 3곳 HTTP 200·CSP·`nosniff`·`no-referrer` 통과
...
- 최신 Sites 저장 버전: 144 (`appgprj_6a92514801988191b79eb9bd314e3fcd~appgver_dcfd30c67f40819194dcad24b9fe1ab0`), 소스 `cfde7e16c9c99d6c65176752155018179504dae1`
- 운영 상태: 서버 오류 로그 개인정보 보완본 버전 107이 공개 운영 중이다. 버전 108~143은 버전 144로 대체해 배포하지 않으며, 상담 FLOW payload 최상위 구조 무결성 보완본 버전 144가 정확한 버전 운영 배포 승인 대기 중이다.
...
먼저 [상담 FLOW payload 최상위 구조 무결성 경계](FLOW_PAYLOAD_STRUCTURE_INTEGRITY_2026_09_05.md)를 확인한다.
...
Sites 버전 144 저장·검증까지 완료했고 공개 운영본은 버전 107이다. 버전 108~143은 버전 144로 대체해 배포하지 않는다. 버전 144 공개 배포에는 정확히 `버전 144 운영 배포 승인`이라는 사용자 명시 승인이 필요하다. 30분 간격 자동 개발은 활성 상태로 다음 고우선순위 감사를 이어간다. 다음 감사는 FLOW 필수 배열의 각 항목이 일반 객체인지 확인하고 보고서·파일·상담·녹취·서류요청·결제·AI 작업·감사기록의 필수 ID·시각·허용 상태·수치가 실제 서버 함수의 기대 형식과 맞는지 검증한다. 배열 껍데기만 정상인 손상 payload는 진행단계·합계·권한·다운로드 로직에 전달하지 않는다. 관리자 안정 ID가 실제로 바뀌는 경우의 결속 초기화는 계정 소유권 확인·운영 D1 백업·감사가 필요한 수동 복구이며 자동화하지 않는다. 실제 파트너 계정·고객 데이터·외부 발송·유료 AI·보관 및 삭제 정책은 별도 승인 없이 사용하거나 변경하지 않는다.
