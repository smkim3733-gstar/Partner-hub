...
- 최신 기능 커밋: `c06b72e251cd4cdfea1a2c2fc47338fc191a37a0` (`fix: make company file facts immutable`)
- 최신 기능: 기업자료 원본 기본행 직접 수정 차단, 저장 키·전체 메타데이터 재검증 뒤 정상 부모 삭제와 자식 원장 연쇄 정리 유지
- 검증: Node 회귀 검사 625개, 격리 workerd/D1/R2 검사 436개, 타입검사, 전체 lint, 변경 파일 포맷 검사, 프로덕션 빌드와 로컬 운영 Worker 화면 3곳 HTTP 200·CSP·`DENY`·`nosniff`·`no-referrer` 통과
...
- 현재 공개 운영본: 버전 107
- 최신 Sites 저장 버전: 174 (`appgprj_6a92514801988191b79eb9bd314e3fcd~appgver_70f456f0dee88191a1c381261278d37a`), 소스 `c06b72e251cd4cdfea1a2c2fc47338fc191a37a0`
- 운영 상태: 서버 오류 로그 개인정보 보완본 버전 107이 공개 운영 중이다. 버전 108~173은 버전 174로 대체해 배포하지 않으며, 기업자료 원본 기본행 불변 보완본 버전 174가 정확한 버전 운영 배포 승인 대기 중이다.
...
먼저 [기업자료 원본 기본행 불변 경계](COMPANY_FILE_OBJECT_IMMUTABILITY_2026_09_05.md)를 확인한다.
...
Sites 버전 174 저장·검증까지 완료했고 공개 운영본은 버전 107이다. 버전 108~173은 버전 174로 대체해 배포하지 않는다. 버전 174 공개 배포에는 정확히 `버전 174 운영 배포 승인`이라는 사용자 명시 승인이 필요하다. 30분 간격 자동 개발은 활성 상태로 다음 고우선순위 감사를 이어간다. 다음 감사는 상담 FLOW 파일 소유권·객체 무결성 원장의 생성 후 수정·삭제 경로와 메타데이터의 정상 목적 전이를 구분하고 D1 경계를 보완한다. 관리자 안정 ID가 실제로 바뀌는 경우의 결속 초기화는 계정 소유권 확인·운영 D1 백업·감사가 필요한 수동 복구이며 자동화하지 않는다. 실제 파트너 계정·고객 데이터·외부 발송·유료 AI·보관 및 삭제 정책은 별도 승인 없이 사용하거나 변경하지 않는다.
