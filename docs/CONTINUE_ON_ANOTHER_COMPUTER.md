...
- 최신 기능 커밋: `f9910d1f0253eb0e26ddc32687288d495afdf08e` (`fix: guard initial FLOW AI jobs`)
- 최신 기능: 상담 FLOW 최초 D1 행의 AI 작업을 `queued/blocked` 무증거 상태로 제한해 완료·실패 작업과 조작 공급자 증거 삽입 차단
- 검증: Node 회귀 검사 650개, 격리 workerd/D1/R2 검사 459개, 타입검사, 전체 lint, 변경 파일 포맷 검사, 프로덕션 빌드와 로컬 운영 Worker 화면 3곳 HTTP 200·CSP·`DENY`·`nosniff`·`no-referrer` 통과
...
- 현재 공개 운영본: 버전 107
- 최신 Sites 저장 버전: 203 (`appgprj_6a92514801988191b79eb9bd314e3fcd~appgver_8931abcf39c08191b1206bb643c08997`), 소스 `f9910d1f0253eb0e26ddc32687288d495afdf08e`
- 운영 상태: 서버 오류 로그 개인정보 보완본 버전 107이 공개 운영 중이다. 버전 108–202는 버전 203으로 대체해 배포하지 않으며, FLOW 최초 AI 작업 삽입 무결성본 버전 203이 정확한 버전 운영 배포 승인 대기 중이다.
...
먼저 [상담 FLOW 최초 AI 작업 삽입 무결성](FLOW_INITIAL_AI_JOB_INTEGRITY_2026_09_06.md)을 확인한다.
...
Sites 버전 203 저장·검증까지 완료했고 공개 운영본은 버전 107이다. 버전 108–202는 버전 203으로 대체해 배포하지 않는다. 버전 203 공개 배포에는 정확히 `버전 203 운영 배포 승인`이라는 사용자 명시 승인이 필요하다. 30분 간격 자동 개발은 활성 상태로 다음 고우선순위 감사를 이어간다. 다음 감사는 증거가 없는 기존 AI 작업도 임의 상태 도약이나 완료·실패 사실 제거를 할 수 없는 전체 작업 수명주기 전이를 확인한다. 관리자 안정 ID가 실제로 바뀌는 경우의 결속 초기화는 계정 소유권 확인·운영 D1 백업·감사가 필요한 수동 복구이며 자동화하지 않는다. 실제 파트너 계정·고객 데이터·외부 발송·유료 AI·보관 및 삭제 정책은 별도 승인 없이 사용하거나 변경하지 않는다.
