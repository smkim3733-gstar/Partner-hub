...
- 최신 기능 커밋: `a85cd3292ffe636a48fa54868d4ffe9f1e00b292` (`fix: require observed AI token usage`)
- 최신 기능: Anthropic 성공 응답의 입력·출력 토큰 증거 필수화와 Step 0 출력 4,000 상한 결속
- 검증: Node 회귀 검사 642개, 격리 workerd/D1/R2 검사 451개, 타입검사, 전체 lint, 변경 파일 포맷 검사, 프로덕션 빌드와 로컬 운영 Worker 화면 3곳 HTTP 200·CSP·`DENY`·`nosniff`·`no-referrer` 통과
...
- 현재 공개 운영본: 버전 107
- 최신 Sites 저장 버전: 189 (`appgprj_6a92514801988191b79eb9bd314e3fcd~appgver_c3482de6d45c819188b1888b0975fb6e`), 소스 `a85cd3292ffe636a48fa54868d4ffe9f1e00b292`
- 운영 상태: 서버 오류 로그 개인정보 보완본 버전 107이 공개 운영 중이다. 버전 108–188은 버전 189로 대체해 배포하지 않으며, AI 토큰 사용량 증거 보완본 버전 189가 정확한 버전 운영 배포 승인 대기 중이다.
...
먼저 [AI 토큰 사용량 증거 무결성 경계](AI_TOKEN_USAGE_EVIDENCE_INTEGRITY_2026_09_06.md)를 확인한다.
...
Sites 버전 189 저장·검증까지 완료했고 공개 운영본은 버전 107이다. 버전 108–188은 버전 189로 대체해 배포하지 않는다. 버전 189 공개 배포에는 정확히 `버전 189 운영 배포 승인`이라는 사용자 명시 승인이 필요하다. 30분 간격 자동 개발은 활성 상태로 다음 고우선순위 감사를 이어간다. 다음 감사는 Worker 중단이나 응답 유실 뒤 `생성중` 원장이 영구 잠금으로 남아 같은 진행의 새 요청을 계속 차단하는지 확인한다. 관리자 안정 ID가 실제로 바뀌는 경우의 결속 초기화는 계정 소유권 확인·운영 D1 백업·감사가 필요한 수동 복구이며 자동화하지 않는다. 실제 파트너 계정·고객 데이터·외부 발송·유료 AI·보관 및 삭제 정책은 별도 승인 없이 사용하거나 변경하지 않는다.
