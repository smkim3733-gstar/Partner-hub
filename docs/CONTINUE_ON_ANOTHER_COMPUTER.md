...
- 최신 기능 커밋: `bb9ffe9cfe8ac9050b54786a1201ad2ae3ce35ad` (`fix: validate AI diagnosis run metadata`)
- 최신 기능: 신규 Step 0 실행의 정확한 UTC 시각·안전한 토큰 수·모델과 지침 재시도 신원 검증
- 검증: Node 회귀 검사 642개, 격리 workerd/D1/R2 검사 450개, 타입검사, 전체 lint, 변경 파일 포맷 검사, 프로덕션 빌드와 로컬 운영 Worker 화면 3곳 HTTP 200·CSP·`DENY`·`nosniff`·`no-referrer` 통과
...
- 현재 공개 운영본: 버전 107
- 최신 Sites 저장 버전: 188 (`appgprj_6a92514801988191b79eb9bd314e3fcd~appgver_b5298f5b813c8191b0e50ed6ae0b3a9d`), 소스 `bb9ffe9cfe8ac9050b54786a1201ad2ae3ce35ad`
- 운영 상태: 서버 오류 로그 개인정보 보완본 버전 107이 공개 운영 중이다. 버전 108–187은 버전 188로 대체해 배포하지 않으며, AI 진단 실행 메타데이터 보완본 버전 188이 정확한 버전 운영 배포 승인 대기 중이다.
...
먼저 [AI 진단 실행 메타데이터 무결성 경계](AI_DIAGNOSIS_RUN_METADATA_INTEGRITY_2026_09_05.md)를 확인한다.
...
Sites 버전 188 저장·검증까지 완료했고 공개 운영본은 버전 107이다. 버전 108–187은 버전 188로 대체해 배포하지 않는다. 버전 188 공개 배포에는 정확히 `버전 188 운영 배포 승인`이라는 사용자 명시 승인이 필요하다. 30분 간격 자동 개발은 활성 상태로 다음 고우선순위 감사를 이어간다. 다음 감사는 성공한 Anthropic 응답에 토큰 사용량이 없거나 0이어도 정상 완료로 기록되는지 확인하고 실제 사용량 증거가 없는 완료 원장을 차단한다. 관리자 안정 ID가 실제로 바뀌는 경우의 결속 초기화는 계정 소유권 확인·운영 D1 백업·감사가 필요한 수동 복구이며 자동화하지 않는다. 실제 파트너 계정·고객 데이터·외부 발송·유료 AI·보관 및 삭제 정책은 별도 승인 없이 사용하거나 변경하지 않는다.
