...
- 최신 기능 커밋: `69d5688f2bc2f90e9f48d7280b3d185cf877a7c5` (`fix: validate FLOW stored filenames`)
- 최신 기능: FLOW 저장 파일명의 업로드 공용 NFC·경로 구분자·제어문자·공백·180자 정규형 결속과 상세·다운로드·SQLite 축소 전 격리
- 검증: Node 회귀 검사 593개, 격리 workerd/D1/R2 검사 398개, 타입검사, 전체 lint, 변경 파일 포맷 검사, 프로덕션 빌드와 로컬 운영 Worker 화면 3곳 HTTP 200·CSP·`DENY`·`nosniff`·`no-referrer` 통과
...
- 현재 공개 운영본: 버전 107
- 최신 Sites 저장 버전: 158 (`appgprj_6a92514801988191b79eb9bd314e3fcd~appgver_956dc91d8850819199b3b600bb25fed1`), 소스 `69d5688f2bc2f90e9f48d7280b3d185cf877a7c5`
- 운영 상태: 서버 오류 로그 개인정보 보완본 버전 107이 공개 운영 중이다. 버전 108~157은 버전 158로 대체해 배포하지 않으며, FLOW 저장 파일명 보완본 버전 158이 정확한 버전 운영 배포 승인 대기 중이다.
...
먼저 [상담 FLOW 파일명 무결성 경계](FLOW_FILE_NAME_INTEGRITY_2026_09_05.md)를 확인한다.
...
Sites 버전 158 저장·검증까지 완료했고 공개 운영본은 버전 107이다. 버전 108~157은 버전 158로 대체해 배포하지 않는다. 버전 158 공개 배포에는 정확히 `버전 158 운영 배포 승인`이라는 사용자 명시 승인이 필요하다. 30분 간격 자동 개발은 활성 상태로 다음 고우선순위 감사를 이어간다. 다음 감사는 한 FLOW의 손상 payload가 같은 `consulting-flow/` 네임스페이스의 다른 진행 파일 ID·키 쌍을 참조할 수 있는지 확인하고 진행별 소유권 또는 전체 ID 고유성 경계를 상세·다운로드·대시보드·저장에 일치시킨다. 관리자 안정 ID가 실제로 바뀌는 경우의 결속 초기화는 계정 소유권 확인·운영 D1 백업·감사가 필요한 수동 복구이며 자동화하지 않는다. 실제 파트너 계정·고객 데이터·외부 발송·유료 AI·보관 및 삭제 정책은 별도 승인 없이 사용하거나 변경하지 않는다.
