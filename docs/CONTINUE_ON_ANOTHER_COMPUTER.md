...
- 최신 기능 커밋: `db42822f54e78cc1171242bcb0c5e4b6ad7b7b3b` (`fix: reject malformed FLOW Unicode`)
- 최신 기능: 전체 FLOW JSON 값·객체 키와 명령·AI 입출력의 짝이 없는 UTF-16 surrogate 저장 차단, SQLite JSON1 문자 변형 전 축소 대시보드 격리
- 검증: Node 회귀 검사 588개, 격리 workerd/D1/R2 검사 378개, 타입검사, 전체 lint, 변경 파일 포맷 검사, 프로덕션 빌드와 로컬 운영 Worker 화면 3곳 HTTP 200·CSP·`DENY`·`nosniff`·`no-referrer` 통과
...
- 최신 Sites 저장 버전: 153 (`appgprj_6a92514801988191b79eb9bd314e3fcd~appgver_ee9926225f188191a35fa7a29cdabda1`), 소스 `db42822f54e78cc1171242bcb0c5e4b6ad7b7b3b`
- 운영 상태: 서버 오류 로그 개인정보 보완본 버전 107이 공개 운영 중이다. 버전 108~152는 버전 153으로 대체해 배포하지 않으며, FLOW 잘못 구성된 Unicode 보완본 버전 153이 정확한 버전 운영 배포 승인 대기 중이다.
...
먼저 [상담 FLOW 잘못 구성된 Unicode 무결성 경계](FLOW_MALFORMED_UNICODE_INTEGRITY_2026_09_05.md)를 확인한다.
...
Sites 버전 153 저장·검증까지 완료했고 공개 운영본은 버전 107이다. 버전 108~152는 버전 153으로 대체해 배포하지 않는다. 버전 153 공개 배포에는 정확히 `버전 153 운영 배포 승인`이라는 사용자 명시 승인이 필요하다. 30분 간격 자동 개발은 활성 상태로 다음 고우선순위 감사를 이어간다. 다음 감사는 정의되지 않은 추가 FLOW 속성이 저장 검증을 통과해 상세 응답의 객체 펼치기나 축소 대시보드에 노출되는지 확인하고 저장 객체·중첩 항목의 허용 키 및 공개 응답 투영을 고정한다. 관리자 안정 ID가 실제로 바뀌는 경우의 결속 초기화는 계정 소유권 확인·운영 D1 백업·감사가 필요한 수동 복구이며 자동화하지 않는다. 실제 파트너 계정·고객 데이터·외부 발송·유료 AI·보관 및 삭제 정책은 별도 승인 없이 사용하거나 변경하지 않는다.
