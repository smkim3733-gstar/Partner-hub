...
- 최신 감사 커밋: `789a4441dfc894722c3c5f6a6d030be300d2b059` (`test: recheck FLOW multipart email change`)
- 최신 확인: 기존 부분 R2 예약 재시도 본문 처리 중 대표가 파트너 로그인 이메일을 바꾸면 기존 ChatGPT 결속을 원자적으로 제거하고 두 번째 신원 검사에서 403으로 차단
- 복구 확인: 거절 중 FLOW·기존 객체·예약을 보존하고 이메일 복구 뒤 같은 안정 ChatGPT 사용자 ID가 정확한 재시도를 보낼 때만 다시 결속되어 같은 명령 ID·파일 ID·R2 키로 완료
...
- 현재 공개 운영본: 버전 107
- 최신 Sites 저장 버전: 277 (`appgprj_6a92514801988191b79eb9bd314e3fcd~appgver_eef2d83b76608191a4d9e477e58b8b61`), 소스 `789a4441dfc894722c3c5f6a6d030be300d2b059`
- 운영 상태: 서버 오류 로그 개인정보 보완본 버전 107이 공개 운영 중이다. 버전 108–276은 더 완전한 후보인 버전 277로 대체한다.
...
먼저 [상담 FLOW multipart 재시도 로그인 이메일 변경 경쟁 무결성](FLOW_RETRY_MULTIPART_EMAIL_CHANGE_RACE_INTEGRITY_2026_09_07.md)을 확인한다.
...
Sites 버전 277 저장·검증까지 완료했고 공개 운영본은 버전 107이다. 버전 108–276은 버전 277로 대체해 배포하지 않는다. 공개 교체는 정확한 `버전 277 운영 배포 승인`이 필요하다. 30분 간격 자동 개발은 활성 상태로 같은 multipart 본문 처리 경쟁에서 파트너 표시명을 변경해도 안정 사용자 ID·회원 ID 결속과 부분 R2 예약 업무 연속성이 보존되는지 감사한다. 실제 파트너 계정·고객 데이터·외부 발송·유료 AI는 별도 승인 없이 사용하지 않는다.
