...
- 최신 기능 커밋: `2c5ba7c3e226ec56d6a8664ce2c660e65af4b0a4` (`fix: bind FLOW report queue effects`)
- 최신 기능: 1차 보고서 생성 요청과 명령 ID 기반 새 작업 한 건의 정확한 효과를 결속
- 검증: Node 회귀 검사 680개, 격리 workerd/D1/R2 검사 502개, 타입검사, 전체 lint, 변경 파일 포맷, 프로덕션 빌드와 로컬 Worker 화면·인증 경계 통과
...
- 현재 공개 운영본: 버전 107
- 최신 Sites 저장 버전: 234 (`appgprj_6a92514801988191b79eb9bd314e3fcd~appgver_c98d47fae7f881919d43fa48fcaac916`), 소스 `2c5ba7c3e226ec56d6a8664ce2c660e65af4b0a4`
- 운영 상태: 서버 오류 로그 개인정보 보완본 버전 107이 공개 운영 중이다. Sites 안전 게이트가 정확한 공개 대상 버전 승인을 요구하며, 버전 108–233은 버전 234로 대체해 배포하지 않는다.
...
먼저 [상담 FLOW 1차 보고서 작업 생성 효과 무결성](FLOW_REPORT_QUEUE_JOB_EFFECT_INTEGRITY_2026_09_06.md)을 확인한다.
...
Sites 버전 234 저장·검증까지 완료했고 공개 운영본은 버전 107이다. 버전 108–233은 버전 234로 대체해 배포하지 않는다. 공개 교체는 Sites 안전 게이트가 요구하는 정확한 `버전 234 운영 배포 승인`이 필요하다. 30분 간격 자동 개발은 활성 상태로 다음 고우선순위 감사를 이어간다. 다음 감사는 `save_source` 명령의 AI 원문과 선택 첨부 효과를 정확히 결속한다. 관리자 안정 ID가 실제로 바뀌는 경우의 결속 초기화는 계정 소유권 확인·운영 D1 백업·감사가 필요한 수동 복구이며 자동화하지 않는다. 실제 파트너 계정·고객 데이터·외부 발송·유료 AI·보관 및 삭제 정책은 별도 승인 없이 사용하거나 변경하지 않는다.
