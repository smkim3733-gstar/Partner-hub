...
- 최신 기능 커밋: `db9615d93b66441c7f2205eecc4bf9fd1b71a333` (`fix: preserve FLOW AI evidence`)
- 최신 기능: 상담 FLOW의 기존 AI 성공·실패 증거, 실패 이력, 작업과 감사기록을 후속 저장에서 변경·삭제하지 못하게 하는 전이 불변 경계
- 검증: Node 회귀 검사 647개, 격리 workerd/D1/R2 검사 451개, 타입검사, 전체 lint, 변경 파일 포맷 검사, 프로덕션 빌드와 로컬 운영 Worker 화면 3곳 HTTP 200·CSP·`DENY`·`nosniff`·`no-referrer` 통과
...
- 현재 공개 운영본: 버전 107
- 최신 Sites 저장 버전: 201 (`appgprj_6a92514801988191b79eb9bd314e3fcd~appgver_7abd842bb47c819197d676a1739ea077`), 소스 `db9615d93b66441c7f2205eecc4bf9fd1b71a333`
- 운영 상태: 서버 오류 로그 개인정보 보완본 버전 107이 공개 운영 중이다. 버전 108–200은 버전 201로 대체해 배포하지 않으며, FLOW AI 증거·감사 전이 불변본 버전 201이 정확한 버전 운영 배포 승인 대기 중이다.
...
먼저 [상담 FLOW AI 증거·감사 전이 불변성](FLOW_AI_EVIDENCE_TRANSITION_IMMUTABILITY_2026_09_06.md)을 확인한다.
...
Sites 버전 201 저장·검증까지 완료했고 공개 운영본은 버전 107이다. 버전 108–200은 버전 201로 대체해 배포하지 않는다. 버전 201 공개 배포에는 정확히 `버전 201 운영 배포 승인`이라는 사용자 명시 승인이 필요하다. 30분 간격 자동 개발은 활성 상태로 다음 고우선순위 감사를 이어간다. 다음 감사는 D1 직접 SQL에서도 같은 AI 증거·감사 불변 전이를 강제하는 저장소 경계를 확인한다. 관리자 안정 ID가 실제로 바뀌는 경우의 결속 초기화는 계정 소유권 확인·운영 D1 백업·감사가 필요한 수동 복구이며 자동화하지 않는다. 실제 파트너 계정·고객 데이터·외부 발송·유료 AI·보관 및 삭제 정책은 별도 승인 없이 사용하거나 변경하지 않는다.
