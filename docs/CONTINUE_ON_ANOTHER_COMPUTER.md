...
- 최신 기능 커밋: `a413336022b3254ff287fa70f315d53d9cd78e18` (`fix: guard company file deletion keys`)
- 최신 기능: 기업자료 삭제 전 저장 키 원장을 확인하고 내구성 삭제 결정에서 재검사해 교차 객체 삭제와 키 변경 경쟁 차단
- 검증: Node 회귀 검사 611개, 격리 workerd/D1/R2 검사 424개, 타입검사, 전체 lint, 변경 파일 포맷 검사, 프로덕션 빌드와 로컬 운영 Worker 화면 3곳 HTTP 200·CSP·`DENY`·`nosniff`·`no-referrer` 통과
...
- 현재 공개 운영본: 버전 107
- 최신 Sites 저장 버전: 167 (`appgprj_6a92514801988191b79eb9bd314e3fcd~appgver_ead9a3f0958081919ee201b629428bf2`), 소스 `a413336022b3254ff287fa70f315d53d9cd78e18`
- 운영 상태: 서버 오류 로그 개인정보 보완본 버전 107이 공개 운영 중이다. 버전 108~166은 버전 167로 대체해 배포하지 않으며, 기업자료 삭제 저장 키 무결성 보완본 버전 167이 정확한 버전 운영 배포 승인 대기 중이다.
...
먼저 [기업자료 삭제 저장 키 무결성 경계](COMPANY_FILE_DELETION_STORAGE_KEY_INTEGRITY_2026_09_05.md)를 확인한다.
...
Sites 버전 167 저장·검증까지 완료했고 공개 운영본은 버전 107이다. 버전 108~166은 버전 167로 대체해 배포하지 않는다. 버전 167 공개 배포에는 정확히 `버전 167 운영 배포 승인`이라는 사용자 명시 승인이 필요하다. 30분 간격 자동 개발은 활성 상태로 다음 고우선순위 감사를 이어간다. 다음 감사는 R2 삭제 성공 뒤 D1 정리 batch가 저장 키가 바뀐 행과 원장을 무조건 제거하지 않는지 확인한다. 관리자 안정 ID가 실제로 바뀌는 경우의 결속 초기화는 계정 소유권 확인·운영 D1 백업·감사가 필요한 수동 복구이며 자동화하지 않는다. 실제 파트너 계정·고객 데이터·외부 발송·유료 AI·보관 및 삭제 정책은 별도 승인 없이 사용하거나 변경하지 않는다.
