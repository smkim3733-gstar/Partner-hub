...
- 최신 기능 커밋: `781c126cd15990443071cb7fdb5eedd7fcd4a922` (`fix: guard AI result storage limits`)
- 최신 기능: AI 결과 안내문 결합 뒤 보고서 상한 재검사, 결과 저장용 보고서·파일·감사기록 용량의 외부 호출 전·확정 전 검사, 공용 FLOW 텍스트 상한
- 검증: Node 회귀 검사 583개, 격리 workerd/D1/R2 검사 370개, 타입검사, 전체 lint, 포맷 검사, 프로덕션 빌드와 로컬 운영 Worker 화면 3곳 HTTP 200·CSP·`DENY`·`nosniff`·`no-referrer` 통과
...
- 최신 Sites 저장 버전: 149 (`appgprj_6a92514801988191b79eb9bd314e3fcd~appgver_e07ac6b32778819184dd49c5242910f6`), 소스 `781c126cd15990443071cb7fdb5eedd7fcd4a922`
- 운영 상태: 서버 오류 로그 개인정보 보완본 버전 107이 공개 운영 중이다. 버전 108~148은 버전 149로 대체해 배포하지 않으며, AI 결과 저장 상한 보완본 버전 149가 정확한 버전 운영 배포 승인 대기 중이다.
...
먼저 [상담 FLOW AI 결과 저장 상한 무결성 경계](FLOW_AI_RESULT_STORAGE_LIMIT_INTEGRITY_2026_09_05.md)를 확인한다.
...
Sites 버전 149 저장·검증까지 완료했고 공개 운영본은 버전 107이다. 버전 108~148은 버전 149로 대체해 배포하지 않는다. 버전 149 공개 배포에는 정확히 `버전 149 운영 배포 승인`이라는 사용자 명시 승인이 필요하다. 30분 간격 자동 개발은 활성 상태로 다음 고우선순위 감사를 이어간다. 다음 감사는 대시보드 SQLite 투영에서 제거되는 파일·AI 작업·감사·명령 수신증 필드의 개별 길이를 투영 전에 확인한다. 상세 조회의 전체 저장 검증과 달리 숨김 필드를 제거하는 축소 투영이 과도한 문자열을 지나치지 않게 한다. 관리자 안정 ID가 실제로 바뀌는 경우의 결속 초기화는 계정 소유권 확인·운영 D1 백업·감사가 필요한 수동 복구이며 자동화하지 않는다. 실제 파트너 계정·고객 데이터·외부 발송·유료 AI·보관 및 삭제 정책은 별도 승인 없이 사용하거나 변경하지 않는다.
