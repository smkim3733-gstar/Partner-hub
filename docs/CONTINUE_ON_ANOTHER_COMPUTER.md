...
- 최신 감사 커밋: `f205380a5c6a579c538aed029ce51970a51cb585` (`test: recheck FLOW multipart suspension`)
- 최신 확인: 기존 부분 R2 예약 재시도 본문 처리 중 대표가 파트너 계정을 정지하면 요청 시작 인증·담당 접근·업로드 권한과 안정 계정 결속이 있어도 두 번째 인증 검사에서 403으로 차단
- 복구 확인: 거절 중 FLOW·기존 객체·예약을 보존하고 대표가 계정을 다시 활성화한 뒤 정확한 재시도만 같은 명령 ID·파일 ID·R2 키로 완료
...
- 현재 공개 운영본: 버전 107
- 최신 Sites 저장 버전: 276 (`appgprj_6a92514801988191b79eb9bd314e3fcd~appgver_aed31e00f2e08191a3459ffeeca0555e`), 소스 `f205380a5c6a579c538aed029ce51970a51cb585`
- 운영 상태: 서버 오류 로그 개인정보 보완본 버전 107이 공개 운영 중이다. 버전 108–275는 더 완전한 후보인 버전 276으로 대체한다.
...
먼저 [상담 FLOW multipart 재시도 계정 정지 경쟁 무결성](FLOW_RETRY_MULTIPART_SUSPENSION_RACE_INTEGRITY_2026_09_07.md)을 확인한다.
...
Sites 버전 276 저장·검증까지 완료했고 공개 운영본은 버전 107이다. 버전 108–275는 버전 276으로 대체해 배포하지 않는다. 공개 교체는 정확한 `버전 276 운영 배포 승인`이 필요하다. 30분 간격 자동 개발은 활성 상태로 같은 multipart 본문 처리 경쟁에서 파트너 로그인 이메일을 변경해 기존 ChatGPT 결속이 폐기돼도 기존 예약 조회·R2 쓰기 전에 두 번째 신원 검사가 차단하는지 감사한다. 실제 파트너 계정·고객 데이터·외부 발송·유료 AI는 별도 승인 없이 사용하지 않는다.
