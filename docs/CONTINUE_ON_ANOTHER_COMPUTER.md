...
- 최신 기능 커밋: `760f0a98660fe95a3e4380833688186723a88fa9` (`fix: isolate FLOW upload cleanup failures`)
- 최신 기능: 상담 녹취의 전사문·보조 음성 업로드 뒤 D1 저장이 실패할 때 두 R2 객체 정리를 독립 실행하고, 남은 객체와 두 예약을 새 명령 ID의 동일 첨부 재시도에서 원래 파일 ID·키로 복구
- 검증: Node 회귀 검사 715개, 격리 workerd/D1/R2 검사 522개, 타입검사, 전체 lint, 변경 파일 포맷, 프로덕션 빌드와 로컬 Worker 화면·인증 경계 통과
...
- 현재 공개 운영본: 버전 107
- 최신 Sites 저장 버전: 258 (`appgprj_6a92514801988191b79eb9bd314e3fcd~appgver_553f256adf348191807663e31763f7e9`), 소스 `760f0a98660fe95a3e4380833688186723a88fa9`
- 운영 상태: 서버 오류 로그 개인정보 보완본 버전 107이 공개 운영 중이다. Sites 안전 게이트가 정확한 공개 대상 버전 승인을 요구하며, 버전 108–257은 버전 258로 대체해 배포하지 않는다.
...
먼저 [상담 FLOW 2슬롯 정리 실패 격리·재시도 무결성](FLOW_DUAL_UPLOAD_CLEANUP_RECOVERY_INTEGRITY_2026_09_06.md)을 확인한다.
...
Sites 버전 258 저장·검증까지 완료했고 공개 운영본은 버전 107이다. 버전 108–257은 버전 258로 대체해 배포하지 않는다. 공개 교체는 정확한 `버전 258 운영 배포 승인`이 필요하다. 30분 간격 자동 개발은 활성 상태로 실패 요청의 권위 FLOW 재조회와 동시 성공 명령 커밋 사이의 R2 정리 시간차 경쟁 감사를 이어간다. 실제 파트너 계정·고객 데이터·외부 발송·유료 AI는 별도 승인 없이 사용하지 않는다.
