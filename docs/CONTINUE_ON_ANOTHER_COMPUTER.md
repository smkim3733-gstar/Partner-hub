...
- 최신 기능 커밋: `71279b1956ee2bc744dc8b78fb030afac908946a` (`fix: revoke access on member state changes`)
- 최신 기능: 정지·재활성화·이메일 변경·삭제와 기존 비밀번호 세션·일회용 설정 링크 폐기를 하나의 D1 트랜잭션으로 처리
- 검증: Node 회귀 검사 470개, 격리 workerd/D1/R2 검사 147개, 타입검사, 전체 lint, 변경 파일 형식검사, 프로덕션 빌드와 로컬 운영 Worker 응답 확인 통과
...
- 최신 Sites 저장 버전: 110 (`appgprj_6a92514801988191b79eb9bd314e3fcd~appgver_74c30c3d8a708191a69ec596f98922b2`), 소스 `71279b1956ee2bc744dc8b78fb030afac908946a`
- 운영 상태: 서버 오류 로그 개인정보 보완본 버전 107이 공개 운영 중이다. 버전 108과 109는 버전 110으로 대체해 배포하지 않으며, 인증수단 폐기 무결성 보완본 버전 110이 공개 배포 승인 대기 중이다.
...
Sites 버전 110 저장·검증까지 완료했고 공개 운영본은 버전 107이다. 버전 110 공개 배포에는 사용자 명시 승인이 필요하다. 승인·배포 점검 뒤 30분 간격 자동 개발이 다음 고우선순위 운영 감사를 이어간다. 실제 파트너 계정·고객 데이터·외부 발송·유료 AI·보관 및 삭제 정책은 별도 승인 없이 사용하거나 변경하지 않는다.
