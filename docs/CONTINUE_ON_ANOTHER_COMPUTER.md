...
- 최신 기능 커밋: `e624fc56d572499ebff52d4a42e184d9492491a5` (`fix: preserve reserved FLOW uploads on failure`)
- 최신 기능: 실패 요청의 재조회 뒤 동일 첨부의 다른 요청이 같은 R2 키를 성공 커밋하는 시간차 경쟁에서 성공 객체가 삭제되지 않도록 예약 객체를 오류 처리에서 보존하고 `pending` 예약 기반 재시도 유지
- 검증: Node 회귀 검사 716개, 격리 workerd/D1/R2 검사 522개, 타입검사, 전체 lint, 변경 파일 포맷, 프로덕션 빌드와 로컬 Worker 화면·인증 경계 통과
...
- 현재 공개 운영본: 버전 107
- 최신 Sites 저장 버전: 259 (`appgprj_6a92514801988191b79eb9bd314e3fcd~appgver_bacb486796dc81918c9c5b0bf616d6f9`), 소스 `e624fc56d572499ebff52d4a42e184d9492491a5`
- 운영 상태: 서버 오류 로그 개인정보 보완본 버전 107이 공개 운영 중이다. 버전 258은 더 깊은 시간차 경쟁을 자동 감사에서 발견해 공개하지 않았고 버전 108–258은 버전 259로 대체한다.
...
먼저 [상담 FLOW 실패 업로드 보존·동시 재시도 무결성](FLOW_UPLOAD_FAILURE_PRESERVATION_INTEGRITY_2026_09_06.md)을 확인한다.
...
Sites 버전 259 저장·검증까지 완료했고 공개 운영본은 버전 107이다. 버전 108–258은 버전 259로 대체해 배포하지 않는다. 공개 교체는 정확한 `버전 259 운영 배포 승인`이 필요하다. 30분 간격 자동 개발은 활성 상태로 2슬롯 중 첫 R2 쓰기 성공 뒤 두 번째 쓰기가 실패하는 보존·재시도 복구 감사를 이어간다. 실제 파트너 계정·고객 데이터·외부 발송·유료 AI는 별도 승인 없이 사용하지 않는다.
