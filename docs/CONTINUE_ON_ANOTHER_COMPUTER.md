...
- 최신 기능 커밋: `4800dc467d9e7e6d2e0918637f58f828fe8994d8` (`fix: bind company files to immutable R2 keys`)
- 최신 기능: 기존 원본 키를 보존하는 가산 D1 원장으로 기업자료 파일 ID와 정확한 R2 저장 키를 결속하고 모든 소비·진단 경로의 교차 객체 참조 차단
- 검증: Node 회귀 검사 609개, 격리 workerd/D1/R2 검사 420개, 타입검사, 전체 lint, 변경 파일 포맷 검사, 프로덕션 빌드와 로컬 운영 Worker 화면 3곳 HTTP 200·CSP·`DENY`·`nosniff`·`no-referrer` 통과
...
- 현재 공개 운영본: 버전 107
- 최신 Sites 저장 버전: 166 (`appgprj_6a92514801988191b79eb9bd314e3fcd~appgver_18f70225b6cc81919ac50b1347f4a282`), 소스 `4800dc467d9e7e6d2e0918637f58f828fe8994d8`
- 운영 상태: 서버 오류 로그 개인정보 보완본 버전 107이 공개 운영 중이다. 버전 108~165는 버전 166으로 대체해 배포하지 않으며, 기업자료 R2 저장 키 무결성 보완본 버전 166이 정확한 버전 운영 배포 승인 대기 중이다.
...
먼저 [기업자료 R2 저장 키 무결성 경계](COMPANY_FILE_STORAGE_KEY_INTEGRITY_2026_09_05.md)를 확인한다.
...
Sites 버전 166 저장·검증까지 완료했고 공개 운영본은 버전 107이다. 버전 108~165는 버전 166으로 대체해 배포하지 않는다. 버전 166 공개 배포에는 정확히 `버전 166 운영 배포 승인`이라는 사용자 명시 승인이 필요하다. 30분 간격 자동 개발은 활성 상태로 다음 고우선순위 감사를 이어간다. 다음 감사는 삭제 결정과 R2 삭제 사이에도 기업 원본 저장 키 원장을 원자적으로 재확인해 키 변경 경쟁으로 다른 객체가 삭제되지 않도록 보완한다. 관리자 안정 ID가 실제로 바뀌는 경우의 결속 초기화는 계정 소유권 확인·운영 D1 백업·감사가 필요한 수동 복구이며 자동화하지 않는다. 실제 파트너 계정·고객 데이터·외부 발송·유료 AI·보관 및 삭제 정책은 별도 승인 없이 사용하거나 변경하지 않는다.
