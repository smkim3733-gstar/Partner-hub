...
- 최신 기능 커밋: `f73b9a10dee131fc69b4b37ae3450dc8d3c2719e` (`fix: classify missing object integrity in inventory`)
- 최신 기능: 원본 보관 목록에서 객체 무결성 원장 누락·MIME 결속 오류를 R2 접근 전 확인 대상으로 분류
- 검증: Node 회귀 검사 607개, 격리 workerd/D1/R2 검사 418개, 타입검사, 전체 lint, 변경 파일 포맷 검사, 프로덕션 빌드와 로컬 운영 Worker 화면 3곳 HTTP 200·CSP·`DENY`·`nosniff`·`no-referrer` 통과
...
- 현재 공개 운영본: 버전 107
- 최신 Sites 저장 버전: 165 (`appgprj_6a92514801988191b79eb9bd314e3fcd~appgver_e415c92571708191ac8204103f46c5e4`), 소스 `f73b9a10dee131fc69b4b37ae3450dc8d3c2719e`
- 운영 상태: 서버 오류 로그 개인정보 보완본 버전 107이 공개 운영 중이다. 버전 108~164는 버전 165로 대체해 배포하지 않으며, 원본 보관 목록 무결성 원장 분류본 버전 165가 정확한 버전 운영 배포 승인 대기 중이다.
...
먼저 [원본 보관 목록 무결성 원장 분류](FILE_INVENTORY_INTEGRITY_LEDGER_CLASSIFICATION_2026_09_05.md)를 확인한다.
...
Sites 버전 165 저장·검증까지 완료했고 공개 운영본은 버전 107이다. 버전 108~164는 버전 165로 대체해 배포하지 않는다. 버전 165 공개 배포에는 정확히 `버전 165 운영 배포 승인`이라는 사용자 명시 승인이 필요하다. 30분 간격 자동 개발은 활성 상태로 다음 고우선순위 감사를 이어간다. 다음 감사는 기업 원본 저장 키를 파일 ID 전용 `company-source/` 경로에 결속해 기존 `metadata` 원본의 교차 객체 참조도 R2 접근 전에 차단하는지 확인한다. 관리자 안정 ID가 실제로 바뀌는 경우의 결속 초기화는 계정 소유권 확인·운영 D1 백업·감사가 필요한 수동 복구이며 자동화하지 않는다. 실제 파트너 계정·고객 데이터·외부 발송·유료 AI·보관 및 삭제 정책은 별도 승인 없이 사용하거나 변경하지 않는다.
