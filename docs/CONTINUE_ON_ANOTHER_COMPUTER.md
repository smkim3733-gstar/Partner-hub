...
- 최신 기능 커밋: `27099de818d002406630113da149ae5e1fe6b44f` (`fix: enforce FLOW AI job lifecycle`)
- 최신 기능: 상담 FLOW AI 작업의 신원 필드, 허용 상태 전이와 상태별 수명주기 필드 조합을 애플리케이션·D1에서 함께 강제
- 검증: Node 회귀 검사 651개, 격리 workerd/D1/R2 검사 462개, 타입검사, 전체 lint, 변경 파일 포맷 검사, 프로덕션 빌드와 로컬 운영 Worker 화면 3곳 HTTP 200·CSP·`DENY`·`nosniff`·`no-referrer` 통과
...
- 현재 공개 운영본: 버전 107
- 최신 Sites 저장 버전: 204 (`appgprj_6a92514801988191b79eb9bd314e3fcd~appgver_55c60f2f014c8191a3d97ad0d4b7b4dc`), 소스 `27099de818d002406630113da149ae5e1fe6b44f`
- 운영 상태: 서버 오류 로그 개인정보 보완본 버전 107이 공개 운영 중이다. 버전 108–203은 버전 204로 대체해 배포하지 않으며, FLOW AI 작업 수명주기 무결성본 버전 204가 정확한 버전 운영 배포 승인 대기 중이다.
...
먼저 [상담 FLOW AI 작업 수명주기 전이 무결성](FLOW_AI_JOB_LIFECYCLE_INTEGRITY_2026_09_06.md)을 확인한다.
...
Sites 버전 204 저장·검증까지 완료했고 공개 운영본은 버전 107이다. 버전 108–203은 버전 204로 대체해 배포하지 않는다. 버전 204 공개 배포에는 정확히 `버전 204 운영 배포 승인`이라는 사용자 명시 승인이 필요하다. 30분 간격 자동 개발은 활성 상태로 다음 고우선순위 감사를 이어간다. 다음 감사는 AI 작업 시작·완료시각을 해당 FLOW revision의 권위 수정시각에 결속한다. 관리자 안정 ID가 실제로 바뀌는 경우의 결속 초기화는 계정 소유권 확인·운영 D1 백업·감사가 필요한 수동 복구이며 자동화하지 않는다. 실제 파트너 계정·고객 데이터·외부 발송·유료 AI·보관 및 삭제 정책은 별도 승인 없이 사용하거나 변경하지 않는다.
