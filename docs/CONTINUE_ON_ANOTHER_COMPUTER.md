...
- 최신 기능 커밋: `1799fe12f50b7676979e7d374cf1b259222544be` (`fix: validate hidden FLOW projection fields`)
- 최신 기능: SQLite 대시보드 축소 투영 전 숨겨지는 보고서·파일·녹취·AI 작업·감사·명령 필드의 타입·길이·중복 ID 검증, 공용 FLOW 필드 상한
- 검증: Node 회귀 검사 584개, 격리 workerd/D1/R2 검사 371개, 타입검사, 전체 lint, 포맷 검사, 프로덕션 빌드와 로컬 운영 Worker 화면 3곳 HTTP 200·CSP·`DENY`·`nosniff`·`no-referrer` 통과
...
- 최신 Sites 저장 버전: 150 (`appgprj_6a92514801988191b79eb9bd314e3fcd~appgver_e4ab9c24cac0819196ff9ad6416f1ee4`), 소스 `1799fe12f50b7676979e7d374cf1b259222544be`
- 운영 상태: 서버 오류 로그 개인정보 보완본 버전 107이 공개 운영 중이다. 버전 108~149는 버전 150으로 대체해 배포하지 않으며, FLOW 숨김 투영 필드 무결성 보완본 버전 150이 정확한 버전 운영 배포 승인 대기 중이다.
...
먼저 [상담 FLOW 숨김 투영 필드 무결성 경계](FLOW_HIDDEN_PROJECTION_INTEGRITY_2026_09_05.md)를 확인한다.
...
Sites 버전 150 저장·검증까지 완료했고 공개 운영본은 버전 107이다. 버전 108~149는 버전 150으로 대체해 배포하지 않는다. 버전 150 공개 배포에는 정확히 `버전 150 운영 배포 승인`이라는 사용자 명시 승인이 필요하다. 30분 간격 자동 개발은 활성 상태로 다음 고우선순위 감사를 이어간다. 다음 감사는 SQLite 축소 투영에서 제거되는 숨김 문자열의 공백 전용 값과 타임스탬프의 실제 날짜 형식을 투영 전에 확인한다. 상세 조회의 전체 저장 검증과 축소 대시보드 조회가 의미상 손상된 숨김 필드에 같은 실패 폐쇄 경계를 적용하게 한다. 관리자 안정 ID가 실제로 바뀌는 경우의 결속 초기화는 계정 소유권 확인·운영 D1 백업·감사가 필요한 수동 복구이며 자동화하지 않는다. 실제 파트너 계정·고객 데이터·외부 발송·유료 AI·보관 및 삭제 정책은 별도 승인 없이 사용하거나 변경하지 않는다.
