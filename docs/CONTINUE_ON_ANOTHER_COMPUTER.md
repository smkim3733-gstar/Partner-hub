...
- 최신 기능 커밋: `340c83d1d9d7571d11322ab803164255bb6848df` (`fix: bind FLOW recording effects`)
- 최신 기능: 녹취 등록 명령과 새 녹취·첨부·4차 AI 작업의 정확한 효과를 결속
- 검증: Node 회귀 검사 679개, 격리 workerd/D1/R2 검사 501개, 타입검사, 전체 lint, 변경 파일 포맷, 프로덕션 빌드와 로컬 Worker 화면·인증 경계 통과
...
- 현재 공개 운영본: 버전 107
- 최신 Sites 저장 버전: 233 (`appgprj_6a92514801988191b79eb9bd314e3fcd~appgver_e895845ea4d481919571700095936b8c`), 소스 `340c83d1d9d7571d11322ab803164255bb6848df`
- 운영 상태: 서버 오류 로그 개인정보 보완본 버전 107이 공개 운영 중이다. Sites 안전 게이트가 정확한 공개 대상 버전 승인을 요구하며, 버전 108–232는 버전 233으로 대체해 배포하지 않는다.
...
먼저 [상담 FLOW 녹취 등록 효과 무결성](FLOW_RECORDING_EFFECT_INTEGRITY_2026_09_06.md)을 확인한다.
...
Sites 버전 233 저장·검증까지 완료했고 공개 운영본은 버전 107이다. 버전 108–232는 버전 233으로 대체해 배포하지 않는다. 공개 교체는 Sites 안전 게이트가 요구하는 정확한 `버전 233 운영 배포 승인`이 필요하다. 30분 간격 자동 개발은 활성 상태로 다음 고우선순위 감사를 이어간다. 다음 감사는 `queue_report1` 명령의 새 1차 작업 한 건과 기존 작업 보존을 정확히 결속한다. 관리자 안정 ID가 실제로 바뀌는 경우의 결속 초기화는 계정 소유권 확인·운영 D1 백업·감사가 필요한 수동 복구이며 자동화하지 않는다. 실제 파트너 계정·고객 데이터·외부 발송·유료 AI·보관 및 삭제 정책은 별도 승인 없이 사용하거나 변경하지 않는다.
