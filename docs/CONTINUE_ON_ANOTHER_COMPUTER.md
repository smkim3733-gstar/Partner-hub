...
- 최신 기능 커밋: `11983cd094c5a05de2ff40af651a8b89514e9c69` (`fix: recheck FLOW access between R2 writes`)
- 최신 확인: 전사문·음성파일 동시 요청의 첫 R2 객체 쓰기 중 계정·권한·신원·담당·사건 상태 변경
- 연속 확인: 수정 전 차단 경쟁의 R2 쓰기 2회 재현, 첫 객체 뒤 접근 재검사로 두 번째 R2와 FLOW D1 차단, 첫 객체·두 `pending` 예약 보존, 복구 뒤 같은 예약으로 한 번 완료, 표시명 변경은 정상 완료
...
- 현재 공개 운영본: 버전 107
- 최신 Sites 저장 버전: 300 (`appgprj_6a92514801988191b79eb9bd314e3fcd~appgver_4e75ff3bfaa48191a29070b54b477f03`), 소스 `11983cd094c5a05de2ff40af651a8b89514e9c69`
- 운영 상태: 서버 오류 로그 개인정보 보완본 버전 107이 공개 운영 중이다. 버전 108–299는 더 완전한 후보인 버전 300으로 대체한다.
...
먼저 [상담 FLOW 다중 R2 쓰기 사이 접근변경 경쟁 무결성](FLOW_BETWEEN_R2_WRITES_ACCESS_RACE_2026_09_07.md)을 확인한다.
...
Sites 버전 300 저장·검증까지 완료했고 공개 운영본은 버전 107이다. 버전 108–299는 버전 300으로 대체해 배포하지 않는다. 공개 교체는 정확한 `버전 300 운영 배포 승인`이 필요하다. 30분 간격 자동 개발은 활성 상태로 첫 R2 객체 쓰기의 스트림 소비 중 상태변경과 저장 예외가 겹칠 때 객체 결과·예약·최종 재검사가 정확한 재시도를 보존하는지 감사한다. 실제 파트너 계정·고객 데이터·외부 발송·유료 AI는 별도 승인 없이 사용하지 않는다.
