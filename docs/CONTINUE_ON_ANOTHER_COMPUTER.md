...
- 최신 기능 커밋: `336fda075e4bf96d85116398eefdf906c0c09c86` (`fix: preserve portal login stat transitions`)
- 최신 기능: 파트너 접속 통계의 계정 ID 불변·UTC 시각 전진·30분 기준 누적 세션 +1 D1 강제
- 검증: Node 회귀 검사 635개, 격리 workerd/D1/R2 검사 444개, 타입검사, 전체 lint, 변경 파일 포맷 검사, 프로덕션 빌드와 로컬 운영 Worker 화면 3곳 HTTP 200·CSP·`DENY`·`nosniff`·`no-referrer` 통과
...
- 현재 공개 운영본: 버전 107
- 최신 Sites 저장 버전: 182 (`appgprj_6a92514801988191b79eb9bd314e3fcd~appgver_eaf0dc6f77b88191b782b8b756ba733e`), 소스 `336fda075e4bf96d85116398eefdf906c0c09c86`
- 운영 상태: 서버 오류 로그 개인정보 보완본 버전 107이 공개 운영 중이다. 버전 108~181은 버전 182로 대체해 배포하지 않으며, 접속 통계 D1 전이 보완본 버전 182가 정확한 버전 운영 배포 승인 대기 중이다.
...
Sites 버전 182 저장·검증까지 완료했고 공개 운영본은 버전 107이다. 버전 108~181은 버전 182로 대체해 배포하지 않는다. 버전 182 공개 배포에는 정확히 `버전 182 운영 배포 승인`이라는 사용자 명시 승인이 필요하다. 30분 간격 자동 개발은 활성 상태로 다음 고우선순위 감사를 이어간다. 다음 감사는 `ai_diagnosis_runs`의 요청 신원·진행 귀속·상태·결과·토큰·생성시각이 `생성중 → 대표 검토 대기/생성실패` 전이로만 저장되는지 모든 D1 쓰기 경로와 수명주기를 점검한다. 관리자 안정 ID가 실제로 바뀌는 경우의 결속 초기화는 계정 소유권 확인·운영 D1 백업·감사가 필요한 수동 복구이며 자동화하지 않는다. 실제 파트너 계정·고객 데이터·외부 발송·유료 AI·보관 및 삭제 정책은 별도 승인 없이 사용하거나 변경하지 않는다.
