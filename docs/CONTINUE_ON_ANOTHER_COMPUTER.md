...
- 최신 기능 커밋: `9fd50c9a84e0615db56c4aa59817f41b145cae80` (`fix: recheck source after copy`)
- 최신 확인: 신청자료 목적지 R2 쓰기 직후 원본 삭제 tombstone과 마지막 FLOW 접근 검사 중 같은 크기 R2 객체 교체
- 연속 확인: 현재 원본 404·원본 무결성 409 차단, FLOW 쓰기·FLOW 0건, 목적지 객체와 `pending` 예약 보존
...
- 현재 공개 운영본: 버전 107
- 최신 Sites 저장 버전: 307 (`appgprj_6a92514801988191b79eb9bd314e3fcd~appgver_d7e3c47dbbe08191abed87cc815ae85e`), 소스 `9fd50c9a84e0615db56c4aa59817f41b145cae80`
- 운영 상태: 서버 오류 로그 개인정보 보완본 버전 107이 공개 운영 중이다. 버전 108–306은 더 완전한 후보인 버전 307로 대체한다.
...
먼저 [상담 FLOW 신청자료 목적지 복사 후 원본 경쟁 무결성](FLOW_INTAKE_POST_COPY_SOURCE_RACE_INTEGRITY_2026_09_07.md)을 확인한다.
...
Sites 버전 307 저장·검증까지 완료했고 공개 운영본은 버전 107이다. 버전 108–306은 버전 307로 대체해 배포하지 않는다. 공개 교체는 정확한 `버전 307 운영 배포 승인`이 필요하다. 30분 간격 자동 개발은 활성 상태로 최종 원본 재검증 직후 FLOW D1 쓰기 중 기업자료 tombstone·D1 원본 사실 변경의 트랜잭션 결속을 다음 순서로 감사한다. R2 `head` 뒤 객체 교체는 외부 저장소 원자성 한계로 별도 분류한다. 실제 파트너 계정·고객 데이터·외부 발송·유료 AI는 별도 승인 없이 사용하지 않는다.
