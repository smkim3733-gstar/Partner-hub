...
- 최신 기능 커밋: `98ee2b488cd12227134a058a15fabbe80ae3f4d1` (`fix: make file ownership ledgers append-only`)
- 최신 기능: 기업자료 최초 담당 계정·진행 연결 원장의 직접 수정·선행 삭제를 D1에서 차단하고 다섯 불변 원장의 운영 코드 직접 변경을 정적 차단
- 검증: Node 회귀 검사 621개, 격리 workerd/D1/R2 검사 434개, 타입검사, 전체 lint, 변경 파일 포맷 검사, 프로덕션 빌드와 로컬 운영 Worker 화면 3곳 HTTP 200·CSP·`DENY`·`nosniff`·`no-referrer` 통과
...
- 현재 공개 운영본: 버전 107
- 최신 Sites 저장 버전: 172 (`appgprj_6a92514801988191b79eb9bd314e3fcd~appgver_5add4f9aeadc81919a3f5861c6b86531`), 소스 `98ee2b488cd12227134a058a15fabbe80ae3f4d1`
- 운영 상태: 서버 오류 로그 개인정보 보완본 버전 107이 공개 운영 중이다. 버전 108~171은 버전 172로 대체해 배포하지 않으며, 기업자료 담당 계정·진행 연결 원장 불변 보완본 버전 172가 정확한 버전 운영 배포 승인 대기 중이다.
...
먼저 [기업자료 담당 계정·진행 연결 원장 불변 경계](COMPANY_FILE_OWNERSHIP_LEDGER_IMMUTABILITY_2026_09_05.md)를 확인한다.
...
Sites 버전 172 저장·검증까지 완료했고 공개 운영본은 버전 107이다. 버전 108~171은 버전 172로 대체해 배포하지 않는다. 버전 172 공개 배포에는 정확히 `버전 172 운영 배포 승인`이라는 사용자 명시 승인이 필요하다. 30분 간격 자동 개발은 활성 상태로 다음 고우선순위 감사를 이어간다. 다음 감사는 업로드 요청의 허용 상태 전이와 삭제 tombstone 보존 계약을 분리하고 저장소 경계를 보완한다. 관리자 안정 ID가 실제로 바뀌는 경우의 결속 초기화는 계정 소유권 확인·운영 D1 백업·감사가 필요한 수동 복구이며 자동화하지 않는다. 실제 파트너 계정·고객 데이터·외부 발송·유료 AI·보관 및 삭제 정책은 별도 승인 없이 사용하거나 변경하지 않는다.
