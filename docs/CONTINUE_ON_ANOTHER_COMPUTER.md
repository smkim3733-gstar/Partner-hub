...
- 최신 감사 커밋: `8535577909b6ea04d554d815803072315d831ecf` (`test: recheck FLOW multipart lifecycle stop`)
- 최신 확인: 생명주기 추적 사건의 기존 부분 R2 예약 재시도 본문 처리 중 대표가 진행을 중단하면 요청 시작 인증·담당 접근·업로드 권한이 있어도 두 번째 생명주기 검사에서 409로 차단
- 복구 확인: 거절 중 FLOW·기존 객체·예약을 보존하고 대표가 진행을 다시 연 뒤 정확한 재시도만 같은 명령 ID·파일 ID·R2 키로 완료
...
- 현재 공개 운영본: 버전 107
- 최신 Sites 저장 버전: 275 (`appgprj_6a92514801988191b79eb9bd314e3fcd~appgver_e8b1944eba188191a8f430e2fbb2e0e1`), 소스 `8535577909b6ea04d554d815803072315d831ecf`
- 운영 상태: 서버 오류 로그 개인정보 보완본 버전 107이 공개 운영 중이다. 버전 108–274는 더 완전한 후보인 버전 275로 대체한다.
...
먼저 [상담 FLOW multipart 재시도 진행 중단 경쟁 무결성](FLOW_RETRY_MULTIPART_LIFECYCLE_RACE_INTEGRITY_2026_09_07.md)을 확인한다.
...
Sites 버전 275 저장·검증까지 완료했고 공개 운영본은 버전 107이다. 버전 108–274는 버전 275로 대체해 배포하지 않는다. 공개 교체는 정확한 `버전 275 운영 배포 승인`이 필요하다. 30분 간격 자동 개발은 활성 상태로 같은 multipart 본문 처리 경쟁에서 파트너 계정을 정지해도 기존 예약 조회·R2 쓰기 전에 두 번째 인증·계정 상태 검사가 차단하는지 감사한다. 실제 파트너 계정·고객 데이터·외부 발송·유료 AI는 별도 승인 없이 사용하지 않는다.
