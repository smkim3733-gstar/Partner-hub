...
- 최신 기능 커밋: `65d3928cc88c532d87583ae2d607edd6b64d0f19` (`fix: make company metadata ledger append-only`)
- 최신 기능: 기업자료 전체 메타데이터 원장의 직접 수정·선행 삭제를 D1에서 차단하고 검증된 부모 삭제의 외래 키 연쇄 정리만 허용
- 검증: Node 회귀 검사 618개, 격리 workerd/D1/R2 검사 432개, 타입검사, 전체 lint, 변경 파일 포맷 검사, 프로덕션 빌드와 로컬 운영 Worker 화면 3곳 HTTP 200·CSP·`DENY`·`nosniff`·`no-referrer` 통과
...
- 현재 공개 운영본: 버전 107
- 최신 Sites 저장 버전: 170 (`appgprj_6a92514801988191b79eb9bd314e3fcd~appgver_693a3b0105bc8191a1237d373d72c974`), 소스 `65d3928cc88c532d87583ae2d607edd6b64d0f19`
- 운영 상태: 서버 오류 로그 개인정보 보완본 버전 107이 공개 운영 중이다. 버전 108~169는 버전 170으로 대체해 배포하지 않으며, 기업자료 메타데이터 원장 불변 보완본 버전 170이 정확한 버전 운영 배포 승인 대기 중이다.
...
먼저 [기업자료 메타데이터 원장 불변 경계](COMPANY_FILE_METADATA_LEDGER_IMMUTABILITY_2026_09_05.md)를 확인한다.
...
Sites 버전 170 저장·검증까지 완료했고 공개 운영본은 버전 107이다. 버전 108~169는 버전 170으로 대체해 배포하지 않는다. 버전 170 공개 배포에는 정확히 `버전 170 운영 배포 승인`이라는 사용자 명시 승인이 필요하다. 30분 간격 자동 개발은 활성 상태로 다음 고우선순위 감사를 이어간다. 다음 감사는 기업자료 저장 키·R2 객체 무결성 원장 자체의 직접 변경 경로를 전수 검사하고 정상 운영에서 INSERT와 부모 연쇄 삭제만 허용되는 불변 경계를 안전하게 적용할 수 있는지 확인한다. 관리자 안정 ID가 실제로 바뀌는 경우의 결속 초기화는 계정 소유권 확인·운영 D1 백업·감사가 필요한 수동 복구이며 자동화하지 않는다. 실제 파트너 계정·고객 데이터·외부 발송·유료 AI·보관 및 삭제 정책은 별도 승인 없이 사용하거나 변경하지 않는다.
