...
Sites 버전 270 저장·검증까지 완료했고 공개 운영본은 버전 107이다. 버전 108–269는 버전 270으로 대체해 배포하지 않는다. 공개 교체는 정확한 `버전 270 운영 배포 승인`이 필요하다. 30분 간격 자동 개발은 활성 상태로 부분 R2 실패 뒤 파트너 표시명이나 로그인 이메일이 변경되면 동일 계정 ID의 정확한 재시도만 기존 예약을 이용하고 이전 인증정보는 차단되는지 감사한다. 실제 파트너 계정·고객 데이터·외부 발송·유료 AI는 별도 승인 없이 사용하지 않는다.
