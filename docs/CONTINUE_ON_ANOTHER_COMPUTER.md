...
- 최신 기능 커밋: `9d1a08b80e3711b63d810167254ccf7c1c1bd309` (`fix: bind FLOW payment confirmation effects`)
- 최신 기능: 입금 확인을 명령 기반 입금 한 건, 대표 신원, 정규 금액·입금일·참조문구, 서버 기록시각과 최초 수행 시작시각에 결속
- 검증: Node 회귀 검사 695개, 관련 FLOW 검사 143개, 집중 검사 101개, 격리 workerd/D1/R2 검사 517개, 타입검사, 전체 lint, 변경 파일 포맷, 프로덕션 빌드와 로컬 Worker 화면·인증 경계 통과
...
- 현재 공개 운영본: 버전 107
- 최신 Sites 저장 버전: 249 (`appgprj_6a92514801988191b79eb9bd314e3fcd~appgver_d38fda332cc4819181de7c3eb8402c28`), 소스 `9d1a08b80e3711b63d810167254ccf7c1c1bd309`
- 운영 상태: 서버 오류 로그 개인정보 보완본 버전 107이 공개 운영 중이다. Sites 안전 게이트가 정확한 공개 대상 버전 승인을 요구하며, 버전 108–248은 버전 249로 대체해 배포하지 않는다.
...
먼저 [상담 FLOW 입금 확인 효과 무결성](FLOW_PAYMENT_CONFIRMATION_EFFECT_INTEGRITY_2026_09_06.md)을 확인한다.
...
Sites 버전 249 저장·검증까지 완료했고 공개 운영본은 버전 107이다. 버전 108–248은 버전 249로 대체해 배포하지 않는다. 공개 교체는 정확한 `버전 249 운영 배포 승인`이 필요하다. 30분 간격 자동 개발은 활성 상태로 다음 `start_aftercare` 효과 감사를 이어간다. 실제 파트너 계정·고객 데이터·외부 발송·유료 AI는 별도 승인 없이 사용하지 않는다.
