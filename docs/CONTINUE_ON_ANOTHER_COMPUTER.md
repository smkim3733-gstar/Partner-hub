...
- 최신 기능 커밋: `65ad199a5174b83825339a8c73093dd672966db5` (`fix: bind company files to R2 integrity`)
- 최신 기능: 신규 기업자료 원본의 R2 ETag·고정 MIME 결속, 같은 크기 바이트·객체 MIME 교체 차단과 가산 `0017` 마이그레이션
...
- 현재 공개 운영본: 버전 107
- 최신 Sites 저장 버전: 163 (`appgprj_6a92514801988191b79eb9bd314e3fcd~appgver_7d815cc3795c81918d5f744bfda4bef5`), 소스 `65ad199a5174b83825339a8c73093dd672966db5`
- 운영 상태: 서버 오류 로그 개인정보 보완본 버전 107이 공개 운영 중이다. 버전 108~162는 버전 163으로 대체해 배포하지 않으며, 기업자료 R2 객체 무결성 보완본 버전 163이 정확한 버전 운영 배포 승인 대기 중이다.
...
먼저 [기업자료 R2 객체 무결성 경계](COMPANY_FILE_R2_OBJECT_INTEGRITY_2026_09_05.md)를 확인한다.
...
Sites 버전 163 저장·검증까지 완료했고 공개 운영본은 버전 107이다. 버전 108~162는 버전 163으로 대체해 배포하지 않는다. 버전 163 공개 배포에는 정확히 `버전 163 운영 배포 승인`이라는 사용자 명시 승인이 필요하다. 30분 간격 자동 개발은 활성 상태로 다음 고우선순위 감사를 이어간다. 다음 감사는 대표용 원본 보관 현황이 객체 무결성 원장의 누락·불일치를 정확히 진단하는지 확인한다. 관리자 안정 ID가 실제로 바뀌는 경우의 결속 초기화는 계정 소유권 확인·운영 D1 백업·감사가 필요한 수동 복구이며 자동화하지 않는다. 실제 파트너 계정·고객 데이터·외부 발송·유료 AI·보관 및 삭제 정책은 별도 승인 없이 사용하거나 변경하지 않는다.
