...
먼저 [상담 FLOW 최초 첨부 동시성·실패 복구](FLOW_INITIAL_ATTACHMENT_CONCURRENCY_RECOVERY_2026_09_06.md)를 확인한다.
...
Sites 버전 250 저장·검증까지 완료했고 공개 운영본은 버전 107이다. 버전 108–249는 버전 250으로 대체해 배포하지 않는다. 공개 교체는 정확한 `버전 250 운영 배포 승인`이 필요하다. 30분 간격 자동 개발은 활성 상태로 21개 지원 명령의 전용 효과 경계 완결성 감사를 이어간다. 실제 파트너 계정·고객 데이터·외부 발송·유료 AI는 별도 승인 없이 사용하지 않는다.
