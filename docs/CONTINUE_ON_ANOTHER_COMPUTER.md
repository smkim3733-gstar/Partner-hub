...
- 최신 기능 커밋: `7652e97480cacf76d95711e90d89cc98ee9e2943` (`fix: reject unexpected AI content blocks`)
- 최신 기능: 도구 미요청 Anthropic 성공 응답의 빈·비텍스트·손상 content block 차단
...
- 현재 공개 운영본: 버전 107
- 최신 Sites 저장 버전: 194 (`appgprj_6a92514801988191b79eb9bd314e3fcd~appgver_39b8ceb930c081918e11df015f073a46`), 소스 `7652e97480cacf76d95711e90d89cc98ee9e2943`
- 운영 상태: 서버 오류 로그 개인정보 보완본 버전 107이 공개 운영 중이다. 버전 108–193은 버전 194로 대체해 배포하지 않으며, AI content block 보완본 버전 194가 정확한 버전 운영 배포 승인 대기 중이다.
...
먼저 [AI content block 무결성 경계](AI_CONTENT_BLOCK_INTEGRITY_2026_09_06.md)를 확인한다.
...
Sites 버전 194 저장·검증까지 완료했고 공개 운영본은 버전 107이다. 버전 108–193은 버전 194로 대체해 배포하지 않는다. 버전 194 공개 배포에는 정확히 `버전 194 운영 배포 승인`이라는 사용자 명시 승인이 필요하다. 30분 간격 자동 개발은 활성 상태로 다음 고우선순위 감사를 이어간다. 다음 감사는 상담 FLOW AI 결과의 공급자 요청·모델·메시지·토큰 증거 보존을 확인한다. 관리자 안정 ID가 실제로 바뀌는 경우의 결속 초기화는 계정 소유권 확인·운영 D1 백업·감사가 필요한 수동 복구이며 자동화하지 않는다. 실제 파트너 계정·고객 데이터·외부 발송·유료 AI·보관 및 삭제 정책은 별도 승인 없이 사용하거나 변경하지 않는다.
