...
- 최신 기능 커밋: `03d901b604bb55d069866bb5aef46aaa23a3bcc9` (`fix: recheck FLOW access after R2 failure`)
- 최신 확인: 첫 R2 스트림 소비 중 `fileUpload` 권한 회수와 객체 생성 전 실패·객체 생성 후 모호한 응답 실패
- 연속 확인: 수정 전 두 경쟁의 503 반환 재현, 저장 실패 직후 접근 재검사로 403 우선 차단, 객체 0개·1개 결과와 두 `pending` 예약 보존, 권한 복구 뒤 같은 예약으로 한 번 완료
- 검증: Node 회귀 검사 754개, 격리 workerd/D1/R2 검사 522개, 타입검사, 전체 lint, 변경 파일 포맷, 프로덕션 빌드와 로컬 Worker 화면·인증 경계 통과
...
- 현재 공개 운영본: 버전 107
- 최신 Sites 저장 버전: 301 (`appgprj_6a92514801988191b79eb9bd314e3fcd~appgver_4fb5516a2e60819191d6f1bdc189ae5d`), 소스 `03d901b604bb55d069866bb5aef46aaa23a3bcc9`
- 운영 상태: 서버 오류 로그 개인정보 보완본 버전 107이 공개 운영 중이다. 버전 108–300은 더 완전한 후보인 버전 301로 대체한다.
...
먼저 [상담 FLOW R2 저장 실패 중 접근변경 경쟁 무결성](FLOW_R2_WRITE_FAILURE_ACCESS_RACE_2026_09_07.md)을 확인한다.
...
Sites 버전 301 저장·검증까지 완료했고 공개 운영본은 버전 107이다. 버전 108–300은 버전 301로 대체해 배포하지 않는다. 공개 교체는 정확한 `버전 301 운영 배포 승인`이 필요하다. 30분 간격 자동 개발은 활성 상태로 첫 객체가 이미 저장된 뒤 두 번째 R2 스트림에서 상태변경과 저장 실패가 겹칠 때 두 객체 결과·예약·FLOW가 안전하게 복구되는지 감사한다. 실제 파트너 계정·고객 데이터·외부 발송·유료 AI는 별도 승인 없이 사용하지 않는다.
