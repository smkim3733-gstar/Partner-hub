...
- 최신 기능 커밋: `ee5399eabf2a9e0f68d83fab229a38a9a4c724ac` (`fix: bind FLOW commands to array targets`)
- 최신 기능: 상담 FLOW 배열 추가 ID·수정 대상·허용 필드를 명령 action에 결속
- 검증: Node 회귀 검사 669개, 격리 workerd/D1/R2 검사 485개, 타입검사, 전체 lint, 변경 파일 포맷 검사와 프로덕션 빌드 통과. 프로덕션 Worker 기동 확인. 별도 화면 HTTP 조회는 Codex 실행 크레딧 제한으로 미완료
...
- 현재 공개 운영본: 버전 107
- 최신 Sites 저장 버전: 219 (`appgprj_6a92514801988191b79eb9bd314e3fcd~appgver_9958cd6bb57881919d192384ed8f0728`), 소스 `ee5399eabf2a9e0f68d83fab229a38a9a4c724ac`
- 운영 상태: 서버 오류 로그 개인정보 보완본 버전 107이 공개 운영 중이다. Sites 안전 게이트가 정확한 공개 대상 버전 승인을 요구하며, 버전 108–218은 버전 219로 대체해 배포하지 않는다.
...
먼저 [상담 FLOW 명령 배열 대상 무결성](FLOW_COMMAND_ARRAY_TARGET_INTEGRITY_2026_09_06.md)을 확인한다.
...
Sites 버전 219 저장·검증까지 완료했고 공개 운영본은 버전 107이다. 버전 108–218은 버전 219로 대체해 배포하지 않는다. 공개 교체는 Sites 안전 게이트가 요구하는 정확한 버전 승인이 필요하다. 30분 간격 자동 개발은 활성 상태로 다음 고우선순위 감사를 이어간다. 다음 감사는 새 명령이 없는 AI 작업 전이가 작업·보고서·감사 외 업무 상태를 함께 바꾸지 못하도록 전이 범위를 확인한다. 관리자 안정 ID가 실제로 바뀌는 경우의 결속 초기화는 계정 소유권 확인·운영 D1 백업·감사가 필요한 수동 복구이며 자동화하지 않는다. 실제 파트너 계정·고객 데이터·외부 발송·유료 AI·보관 및 삭제 정책은 별도 승인 없이 사용하거나 변경하지 않는다.
