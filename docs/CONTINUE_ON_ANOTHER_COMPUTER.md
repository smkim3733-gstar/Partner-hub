...
- 최신 감사 커밋: `1766bd39650e6f5cc5467f68b98396c01dd9bda6` (`test: cover same-command FLOW retry`)
- 최신 확인: 2슬롯 부분 저장 실패 뒤 원래 명령 ID와 동일 첨부 재시도가 기존 두 파일 ID·키를 재사용하고 명령·예약·완료 원장을 같은 ID에 결속
- 검증: Node 회귀 검사 721개, 격리 workerd/D1/R2 검사 522개, 타입검사, 전체 lint, 변경 파일 포맷, 프로덕션 빌드와 로컬 Worker 화면·인증 경계 통과
...
- 현재 공개 운영본: 버전 107
- 최신 Sites 저장 버전: 264 (`appgprj_6a92514801988191b79eb9bd314e3fcd~appgver_9901145b9e208191b7bcccb9f38cda7f`), 소스 `1766bd39650e6f5cc5467f68b98396c01dd9bda6`
- 운영 상태: 서버 오류 로그 개인정보 보완본 버전 107이 공개 운영 중이다. 버전 108–263은 더 완전한 후보인 버전 264로 대체한다.
...
먼저 [상담 FLOW 동일 명령 첨부 재시도 무결성](FLOW_SAME_COMMAND_UPLOAD_RETRY_INTEGRITY_2026_09_06.md)을 확인한다.
...
Sites 버전 264 저장·검증까지 완료했고 공개 운영본은 버전 107이다. 버전 108–263은 버전 264로 대체해 배포하지 않는다. 공개 교체는 정확한 `버전 264 운영 배포 승인`이 필요하다. 30분 간격 자동 개발은 활성 상태로 부분 R2 실패 뒤 같은 명령 ID의 첨부 변경 재시도가 기존 객체를 덮어쓰지 못하는지 감사한다. 실제 파트너 계정·고객 데이터·외부 발송·유료 AI는 별도 승인 없이 사용하지 않는다.
