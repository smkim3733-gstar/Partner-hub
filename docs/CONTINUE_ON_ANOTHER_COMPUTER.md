...
- 최신 감사 커밋: `96d0095a9bd1fae874a91ae9586545b63f7fbd88` (`fix: recheck FLOW case authority`)
- 최신 확인: 파트너 부분 R2 저장 뒤 사건 담당 계정 불일치 또는 진행 중단이 발생한 재시도는 R2 쓰기 전 409로 차단하고 FLOW·기존 객체·예약 보존
- 최신 보완: 기존 FLOW 담당자와 포털 사건의 명시적 담당 ID가 다른 비정상 상태를 접근 시 409로 격리하며, 권한 없는 다른 계정의 기존 403과 담당 ID 없는 레거시 FLOW 접근은 유지
- 검증: Node 회귀 검사 725개, 격리 workerd/D1/R2 검사 522개, 타입검사, 전체 lint, 변경 파일 포맷, 프로덕션 빌드와 로컬 Worker 화면·인증 경계 통과
...
- 현재 공개 운영본: 버전 107
- 최신 Sites 저장 버전: 269 (`appgprj_6a92514801988191b79eb9bd314e3fcd~appgver_e69cd6767c2c81918d5baf62049eb017`), 소스 `96d0095a9bd1fae874a91ae9586545b63f7fbd88`
- 운영 상태: 서버 오류 로그 개인정보 보완본 버전 107이 공개 운영 중이다. 버전 108–268은 더 완전한 후보인 버전 269로 대체한다.
...
먼저 [상담 FLOW 재시도 담당·진행상태 무결성](FLOW_RETRY_ASSIGNMENT_LIFECYCLE_INTEGRITY_2026_09_07.md)을 확인한다.
...
Sites 버전 269 저장·검증까지 완료했고 공개 운영본은 버전 107이다. 버전 108–268은 버전 269로 대체해 배포하지 않는다. 공개 교체는 정확한 `버전 269 운영 배포 승인`이 필요하다. 30분 간격 자동 개발은 활성 상태로 부분 R2 실패 뒤 사건 삭제나 담당 파트너 계정 삭제가 발생한 이전 재시도가 R2 객체와 예약을 건드리지 못하는지 감사한다. 실제 파트너 계정·고객 데이터·외부 발송·유료 AI는 별도 승인 없이 사용하지 않는다.
