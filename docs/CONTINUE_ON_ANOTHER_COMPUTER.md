...
- 최신 기능 커밋: `b5857542d088c17713f0df100ed4c9b1984fc7e8` (`fix: bind FLOW files to case owners`)
- 최신 기능: FLOW 파일 ID·R2 키의 D1 진행 소유권 결속, 원자적 신규 등록, 충돌 안전 백필과 교차 진행 참조 격리
- 검증: Node 회귀 검사 594개, 격리 workerd/D1/R2 검사 402개, 타입검사, 전체 lint, 변경 파일 포맷 검사, 프로덕션 빌드와 로컬 운영 Worker 화면 3곳 HTTP 200·CSP·`DENY`·`nosniff`·`no-referrer` 통과
...
- 현재 공개 운영본: 버전 107
- 최신 Sites 저장 버전: 159 (`appgprj_6a92514801988191b79eb9bd314e3fcd~appgver_5caab1c702c48191902fd3be861a7860`), 소스 `b5857542d088c17713f0df100ed4c9b1984fc7e8`
- 운영 상태: 서버 오류 로그 개인정보 보완본 버전 107이 공개 운영 중이다. 버전 108~158은 버전 159로 대체해 배포하지 않으며, FLOW 파일 진행 소유권 보완본 버전 159가 정확한 버전 운영 배포 승인 대기 중이다.
...
먼저 [상담 FLOW 파일 진행 소유권 무결성 경계](FLOW_FILE_CASE_OWNERSHIP_INTEGRITY_2026_09_05.md)를 확인한다.
...
Sites 버전 159 저장·검증까지 완료했고 공개 운영본은 버전 107이다. 버전 108~158은 버전 159로 대체해 배포하지 않는다. 버전 159 공개 배포에는 정확히 `버전 159 운영 배포 승인`이라는 사용자 명시 승인이 필요하다. 30분 간격 자동 개발은 활성 상태로 다음 고우선순위 감사를 이어간다. 다음 감사는 FLOW 파일의 이름·MIME·크기·용도·생성시각을 최초 원장 사실에 결속하고 상세·다운로드·대시보드·저장 전이에서 일치시키는 경계다. 관리자 안정 ID가 실제로 바뀌는 경우의 결속 초기화는 계정 소유권 확인·운영 D1 백업·감사가 필요한 수동 복구이며 자동화하지 않는다. 실제 파트너 계정·고객 데이터·외부 발송·유료 AI·보관 및 삭제 정책은 별도 승인 없이 사용하거나 변경하지 않는다.
