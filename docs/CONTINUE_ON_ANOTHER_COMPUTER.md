...
- 최신 기능 커밋: `963ae8a8cb0116d0ded5ea84ffbc7b8b210bf9f5` (`fix: retain FLOW AI failure evidence`)
- 최신 기능: 상담 FLOW 공급자 4xx·5xx의 지침·요청 모델·HTTP 상태·검증된 요청 ID 원자 저장
...
- 현재 공개 운영본: 버전 107
- 최신 Sites 저장 버전: 196 (`appgprj_6a92514801988191b79eb9bd314e3fcd~appgver_5e508f4084148191a91e08eaf9342d4c`), 소스 `963ae8a8cb0116d0ded5ea84ffbc7b8b210bf9f5`
- 운영 상태: 서버 오류 로그 개인정보 보완본 버전 107이 공개 운영 중이다. 버전 108–195는 버전 196으로 대체해 배포하지 않으며, FLOW AI 실패 증거 보완본 버전 196이 정확한 버전 운영 배포 승인 대기 중이다.
...
먼저 [상담 FLOW AI 실패 증거 무결성 경계](FLOW_AI_FAILURE_EVIDENCE_INTEGRITY_2026_09_06.md)를 확인한다.
...
Sites 버전 196 저장·검증까지 완료했고 공개 운영본은 버전 107이다. 버전 108–195는 버전 196으로 대체해 배포하지 않는다. 버전 196 공개 배포에는 정확히 `버전 196 운영 배포 승인`이라는 사용자 명시 승인이 필요하다. 30분 간격 자동 개발은 활성 상태로 다음 고우선순위 감사를 이어간다. 다음 감사는 수동 재시도 뒤에도 과거 공급자 실패 증거가 사라지지 않는지 확인한다. 관리자 안정 ID가 실제로 바뀌는 경우의 결속 초기화는 계정 소유권 확인·운영 D1 백업·감사가 필요한 수동 복구이며 자동화하지 않는다. 실제 파트너 계정·고객 데이터·외부 발송·유료 AI·보관 및 삭제 정책은 별도 승인 없이 사용하거나 변경하지 않는다.
