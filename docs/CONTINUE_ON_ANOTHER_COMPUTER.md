...
- 최신 감사 커밋: `50a6ae47795b559a4f335279561fb09e36d58aea` (`test: guard FLOW upload commit against suspended account`)
- 최신 확인: 기존 부분 R2 예약의 정확한 재시도가 두 객체와 최종 접근 검사를 마친 뒤 D1 FLOW 쓰기 직전에 실제 관리자 경로로 파트너 계정을 정지
- 연속 확인: 포털 상태 payload 조건부 비교가 정지 전 FLOW·영수증·파일 원장 커밋을 차단하고 D1 전체를 원자적으로 되돌리며 두 객체·두 `pending` 예약을 보존하고, 재활성화 뒤 같은 파일 ID·R2 키로 한 번 완료
- 검증: Node 회귀 검사 737개, 격리 workerd/D1/R2 검사 522개, 타입검사, 전체 lint, 변경 파일 포맷, 프로덕션 빌드와 로컬 Worker 화면·인증 경계 통과
...
- 현재 공개 운영본: 버전 107
- 최신 Sites 저장 버전: 287 (`appgprj_6a92514801988191b79eb9bd314e3fcd~appgver_1e0a653f10e88191b3f60bcbc2a224a5`), 소스 `50a6ae47795b559a4f335279561fb09e36d58aea`
- 운영 상태: 서버 오류 로그 개인정보 보완본 버전 107이 공개 운영 중이다. 버전 108–286은 더 완전한 후보인 버전 287로 대체한다.
...
먼저 [상담 FLOW D1 쓰기 직전 계정 정지 경쟁 무결성](FLOW_RETRY_D1_WRITE_SUSPENSION_RACE_INTEGRITY_2026_09_07.md)을 확인한다.
