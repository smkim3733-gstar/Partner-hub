...
- 최신 기능 커밋: `dd3092c1e3d17f551f69817dcbd151399802e481` (`fix: surface company object integrity in inventory`)
- 최신 기능: 대표 전용 원본 보관 현황의 ETag·MIME·원장 불일치 진단과 기존 원본 검증 범위 명시
- 검증: Node 회귀 검사 607개, 격리 workerd/D1/R2 검사 417개, 타입검사, 전체 lint, 변경 파일 포맷 검사, 프로덕션 빌드와 로컬 운영 Worker 화면 3곳 HTTP 200·CSP·`DENY`·`nosniff`·`no-referrer` 통과
...
- 현재 공개 운영본: 버전 107
- 최신 Sites 저장 버전: 164 (`appgprj_6a92514801988191b79eb9bd314e3fcd~appgver_486ccaab470c8191a615c4a353af22ec`), 소스 `dd3092c1e3d17f551f69817dcbd151399802e481`
- 운영 상태: 서버 오류 로그 개인정보 보완본 버전 107이 공개 운영 중이다. 버전 108~163은 버전 164로 대체해 배포하지 않으며, 원본 보관 현황 객체 무결성 진단본 버전 164가 정확한 버전 운영 배포 승인 대기 중이다.
...
먼저 [원본 보관 현황 객체 무결성 진단](FILE_INVENTORY_OBJECT_INTEGRITY_DIAGNOSTICS_2026_09_05.md)을 확인한다.
...
Sites 버전 164 저장·검증까지 완료했고 공개 운영본은 버전 107이다. 버전 108~163은 버전 164로 대체해 배포하지 않는다. 버전 164 공개 배포에는 정확히 `버전 164 운영 배포 승인`이라는 사용자 명시 승인이 필요하다. 30분 간격 자동 개발은 활성 상태로 다음 고우선순위 감사를 이어간다. 다음 감사는 보관 목록의 DB 분류도 객체 무결성 원장 누락·구조 불일치를 `메타데이터 확인 필요`로 드러내는지 확인한다. 관리자 안정 ID가 실제로 바뀌는 경우의 결속 초기화는 계정 소유권 확인·운영 D1 백업·감사가 필요한 수동 복구이며 자동화하지 않는다. 실제 파트너 계정·고객 데이터·외부 발송·유료 AI·보관 및 삭제 정책은 별도 승인 없이 사용하거나 변경하지 않는다.
