...
- 최신 기능 커밋: `08749be873981fa03c5f2f81bfbb88053b195db0` (`fix: bound AI diagnosis request locks`)
- 최신 기능: 생성중 Step 0 잠금의 요청 지문 전용 키·256 UTF-8 바이트 계약
- 검증: Node 회귀 검사 641개, 격리 workerd/D1/R2 검사 447개, 타입검사, 전체 lint, 변경 파일 포맷 검사, 프로덕션 빌드와 로컬 운영 Worker 화면 3곳 HTTP 200·CSP·`DENY`·`nosniff`·`no-referrer` 통과
...
- 현재 공개 운영본: 버전 107
- 최신 Sites 저장 버전: 185 (`appgprj_6a92514801988191b79eb9bd314e3fcd~appgver_64e68351b210819199d4ebad5c1ea433`), 소스 `08749be873981fa03c5f2f81bfbb88053b195db0`
- 운영 상태: 서버 오류 로그 개인정보 보완본 버전 107이 공개 운영 중이다. 버전 108~184는 버전 185로 대체해 배포하지 않으며, AI 진단 생성중 잠금 envelope 보완본 버전 185가 정확한 버전 운영 배포 승인 대기 중이다.
...
Sites 버전 185 저장·검증까지 완료했고 공개 운영본은 버전 107이다. 버전 108~184는 버전 185로 대체해 배포하지 않는다. 버전 185 공개 배포에는 정확히 `버전 185 운영 배포 승인`이라는 사용자 명시 승인이 필요하다. 30분 간격 자동 개발은 활성 상태로 다음 고우선순위 감사를 이어간다. 다음 감사는 AI 실행 원장의 요청 ID·진행 ID·기업·지침·모델·실행자 필드가 신규 삽입에서 합리적인 문자 길이 상한을 갖는지 점검한다. 관리자 안정 ID가 실제로 바뀌는 경우의 결속 초기화는 계정 소유권 확인·운영 D1 백업·감사가 필요한 수동 복구이며 자동화하지 않는다. 실제 파트너 계정·고객 데이터·외부 발송·유료 AI·보관 및 삭제 정책은 별도 승인 없이 사용하거나 변경하지 않는다.
