...
- 최신 기능 커밋: `10c5a546e3d3e5f3c7dc6bf42aeb6163be519639` (`fix: enforce upload receipt lifecycle`)
- 최신 기능: 기업자료 업로드 영수증 식별값 고정, 전진 상태 전이 강제, 삭제 tombstone 수정·직접 삭제·지연 재시도 부활 차단
- 검증: Node 회귀 검사 623개, 격리 workerd/D1/R2 검사 435개, 타입검사, 전체 lint, 변경 파일 포맷 검사, 프로덕션 빌드와 로컬 운영 Worker 화면 3곳 HTTP 200·CSP·`DENY`·`nosniff`·`no-referrer` 통과
...
- 현재 공개 운영본: 버전 107
- 최신 Sites 저장 버전: 173 (`appgprj_6a92514801988191b79eb9bd314e3fcd~appgver_652d424ecab081918283239cd5d433cb`), 소스 `10c5a546e3d3e5f3c7dc6bf42aeb6163be519639`
- 운영 상태: 서버 오류 로그 개인정보 보완본 버전 107이 공개 운영 중이다. 버전 108~172는 버전 173으로 대체해 배포하지 않으며, 업로드 영수증 수명주기·tombstone 무결성 보완본 버전 173이 정확한 버전 운영 배포 승인 대기 중이다.
...
먼저 [기업자료 업로드 요청 수명주기·tombstone 무결성 경계](COMPANY_FILE_UPLOAD_REQUEST_LIFECYCLE_INTEGRITY_2026_09_05.md)를 확인한다.
...
Sites 버전 173 저장·검증까지 완료했고 공개 운영본은 버전 107이다. 버전 108~172는 버전 173으로 대체해 배포하지 않는다. 버전 173 공개 배포에는 정확히 `버전 173 운영 배포 승인`이라는 사용자 명시 승인이 필요하다. 30분 간격 자동 개발은 활성 상태로 다음 고우선순위 감사를 이어간다. 다음 감사는 기업자료 원본 기본행이 생성 뒤 직접 변경되는 경로를 전수 확인하고 정상 부모 삭제는 유지하면서 직접 수정을 차단한다. 관리자 안정 ID가 실제로 바뀌는 경우의 결속 초기화는 계정 소유권 확인·운영 D1 백업·감사가 필요한 수동 복구이며 자동화하지 않는다. 실제 파트너 계정·고객 데이터·외부 발송·유료 AI·보관 및 삭제 정책은 별도 승인 없이 사용하거나 변경하지 않는다.
