...
- 최신 기능 커밋: `4dfbae0698400226afbfd9d3b44b1af90e7a50df` (`fix: persist FLOW AI provider evidence`)
- 최신 기능: 상담 FLOW AI 완료 작업의 지침·요청 모델·공급자 요청/응답 모델/Message ID·토큰 증거 원자 저장
...
- 현재 공개 운영본: 버전 107
- 최신 Sites 저장 버전: 195 (`appgprj_6a92514801988191b79eb9bd314e3fcd~appgver_79c6bace16248191a33d6a2d94205377`), 소스 `4dfbae0698400226afbfd9d3b44b1af90e7a50df`
- 운영 상태: 서버 오류 로그 개인정보 보완본 버전 107이 공개 운영 중이다. 버전 108–194는 버전 195로 대체해 배포하지 않으며, FLOW AI 공급자 증거 보완본 버전 195가 정확한 버전 운영 배포 승인 대기 중이다.
...
먼저 [상담 FLOW AI 공급자 증거 무결성 경계](FLOW_AI_PROVIDER_EVIDENCE_INTEGRITY_2026_09_06.md)를 확인한다.
...
Sites 버전 195 저장·검증까지 완료했고 공개 운영본은 버전 107이다. 버전 108–194는 버전 195로 대체해 배포하지 않는다. 버전 195 공개 배포에는 정확히 `버전 195 운영 배포 승인`이라는 사용자 명시 승인이 필요하다. 30분 간격 자동 개발은 활성 상태로 다음 고우선순위 감사를 이어간다. 다음 감사는 상담 FLOW AI 실패 결과의 공급자 오류 요청 ID와 HTTP 상태 증거 보존을 확인한다. 관리자 안정 ID가 실제로 바뀌는 경우의 결속 초기화는 계정 소유권 확인·운영 D1 백업·감사가 필요한 수동 복구이며 자동화하지 않는다. 실제 파트너 계정·고객 데이터·외부 발송·유료 AI·보관 및 삭제 정책은 별도 승인 없이 사용하거나 변경하지 않는다.
