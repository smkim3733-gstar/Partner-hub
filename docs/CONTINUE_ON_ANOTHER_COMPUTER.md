...
Sites 버전 302 저장·검증까지 완료했고 공개 운영본은 버전 107이다. 버전 108–301은 버전 302로 대체해 배포하지 않는다. 공개 교체는 정확한 `버전 302 운영 배포 승인`이 필요하다. 30분 간격 자동 개발은 활성 상태로 신청자료 원본을 FLOW 첨부로 복사하는 R2 쓰기 실패와 접근변경이 겹칠 때 예약·객체·FLOW가 안전하게 복구되는지 감사한다. 실제 파트너 계정·고객 데이터·외부 발송·유료 AI는 별도 승인 없이 사용하지 않는다.
