...
- 최신 기능 커밋: `33ca024fc304d56586b038deaceb5fbc7a7142c8` (`fix: reject unknown FLOW fields`)
- 최신 기능: FLOW 최상위·중첩 객체의 정확 허용 키, 명시적 상세 공개 투영, SQLite 축소 전 숨김 객체의 미정의 속성 격리
- 검증: Node 회귀 검사 589개, 격리 workerd/D1/R2 검사 381개, 타입검사, 전체 lint, 변경 파일 포맷 검사, 프로덕션 빌드와 로컬 운영 Worker 화면 3곳 HTTP 200·CSP·`DENY`·`nosniff`·`no-referrer` 통과
...
- 현재 공개 운영본: 버전 107
- 최신 Sites 저장 버전: 154 (`appgprj_6a92514801988191b79eb9bd314e3fcd~appgver_ac160a67714c8191bf741417ede50e53`), 소스 `33ca024fc304d56586b038deaceb5fbc7a7142c8`
- 운영 상태: 서버 오류 로그 개인정보 보완본 버전 107이 공개 운영 중이다. 버전 108~153은 버전 154로 대체해 배포하지 않으며, FLOW 정확 객체 구조 보완본 버전 154가 정확한 버전 운영 배포 승인 대기 중이다.
...
Sites 버전 154 저장·검증까지 완료했고 공개 운영본은 버전 107이다. 버전 108~153은 버전 154로 대체해 배포하지 않는다. 버전 154 공개 배포에는 정확히 `버전 154 운영 배포 승인`이라는 사용자 명시 승인이 필요하다. 30분 간격 자동 개발은 활성 상태로 다음 고우선순위 감사를 이어간다. 다음 감사는 FLOW 파일 메타데이터가 목적별 업로드 크기 상한을 초과해도 저장 구조 검증을 통과하는지 확인하고 D1/R2 원본 대조 경계를 일치시킨다. 관리자 안정 ID가 실제로 바뀌는 경우의 결속 초기화는 계정 소유권 확인·운영 D1 백업·감사가 필요한 수동 복구이며 자동화하지 않는다. 실제 파트너 계정·고객 데이터·외부 발송·유료 AI·보관 및 삭제 정책은 별도 승인 없이 사용하거나 변경하지 않는다.
