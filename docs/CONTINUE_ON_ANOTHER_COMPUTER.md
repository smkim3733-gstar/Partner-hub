...
- 최신 기능 커밋: `ef1c68db31b1277de4d4b581a4d90c8d3bf3c549` (`fix: preserve suspended registration state`)
- 최신 기능: ChatGPT 자가가입에서 정지 계정 상태를 보존하고, 동일 승인대기 재시도를 무기록 처리하며, 접속 IP·ChatGPT 사용자 ID 기반 반복 가입 제한 적용
- 검증: Node 회귀 검사 467개, 격리 workerd/D1/R2 검사 142개, 타입검사, 전체 lint, 변경 파일 형식검사, 프로덕션 빌드와 로컬 운영 Worker 응답 확인 통과
...
- 최신 Sites 저장 버전: 109 (`appgprj_6a92514801988191b79eb9bd314e3fcd~appgver_fad67ebf1ad0819193504c6e9aa91757`), 소스 `ef1c68db31b1277de4d4b581a4d90c8d3bf3c549`
- 운영 상태: 서버 오류 로그 개인정보 보완본 버전 107이 공개 운영 중이다. 버전 108은 버전 109로 대체해 배포하지 않으며, 가입 계정 상태 무결성 보완본 버전 109가 공개 배포 승인 대기 중이다.
...
Sites 버전 109 저장·검증까지 완료했고 공개 운영본은 버전 107이다. 버전 109 공개 배포에는 사용자 명시 승인이 필요하다. 승인·배포 점검 뒤 30분 간격 자동 개발이 다음 고우선순위 운영 감사를 이어간다. 실제 파트너 계정·고객 데이터·외부 발송·유료 AI·보관 및 삭제 정책은 별도 승인 없이 사용하거나 변경하지 않는다.
