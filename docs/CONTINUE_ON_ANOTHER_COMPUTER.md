...
- 최신 기능 커밋: `4e579d0b143a607f5326e5c04d9d42b43487483a` (`fix: recover partial FLOW storage writes`)
- 최신 기능: 전사문·보조 음성 중 첫 R2 쓰기만 성공한 부분 저장에서 첫 객체와 두 예약을 보존하고, 안전한 503 뒤 새 명령 ID의 동일 첨부 재시도가 기존 파일 ID·키로 완성
- 검증: Node 회귀 검사 717개, 격리 workerd/D1/R2 검사 522개, 타입검사, 전체 lint, 변경 파일 포맷, 프로덕션 빌드와 로컬 Worker 화면·인증 경계 통과
...
- 현재 공개 운영본: 버전 107
- 최신 Sites 저장 버전: 260 (`appgprj_6a92514801988191b79eb9bd314e3fcd~appgver_89587622a798819191304384a0f282b0`), 소스 `4e579d0b143a607f5326e5c04d9d42b43487483a`
- 운영 상태: 서버 오류 로그 개인정보 보완본 버전 107이 공개 운영 중이다. 버전 108–259는 더 완전한 후보인 버전 260으로 대체한다.
...
먼저 [상담 FLOW 부분 R2 쓰기 실패 복구 무결성](FLOW_PARTIAL_R2_WRITE_RECOVERY_INTEGRITY_2026_09_06.md)을 확인한다.
...
Sites 버전 260 저장·검증까지 완료했고 공개 운영본은 버전 107이다. 버전 108–259는 버전 260으로 대체해 배포하지 않는다. 공개 교체는 정확한 `버전 260 운영 배포 승인`이 필요하다. 30분 간격 자동 개발은 활성 상태로 R2가 객체를 실제 저장한 뒤 오류 응답만 발생하는 모호한 쓰기 결과의 정확 재시도 수렴 감사를 이어간다. 실제 파트너 계정·고객 데이터·외부 발송·유료 AI는 별도 승인 없이 사용하지 않는다.
