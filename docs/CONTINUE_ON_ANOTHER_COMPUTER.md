...
- 최신 기능 커밋: `78498fe1dbec8147ba2786136b020eff816af59f` (`fix: verify original size before download`)
- 최신 기능: 기업자료 다운로드의 D1 원장 크기와 R2 실제 객체 크기를 응답 전후에 대조하고 조회 중 변경·손상된 본문 반환 차단
- 검증: Node 회귀 검사 560개, 격리 workerd/D1/R2 검사 350개, 타입검사, 전체 lint, 프로덕션 빌드와 로컬 운영 Worker 화면 3곳 HTTP 200·CSP·`nosniff`·`no-referrer` 통과
...
- 최신 Sites 저장 버전: 140 (`appgprj_6a92514801988191b79eb9bd314e3fcd~appgver_ea9b185a99708191af07d70613d88cf7`), 소스 `78498fe1dbec8147ba2786136b020eff816af59f`
- 운영 상태: 서버 오류 로그 개인정보 보완본 버전 107이 공개 운영 중이다. 버전 108~139는 버전 140으로 대체해 배포하지 않으며, 기업자료 다운로드 원본 크기 무결성 보완본 버전 140이 정확한 버전 운영 배포 승인 대기 중이다.
...
먼저 [기업자료 다운로드 원본 크기 무결성 경계](COMPANY_FILE_DOWNLOAD_OBJECT_INTEGRITY_2026_09_05.md)를 확인한다.
...
Sites 버전 140 저장·검증까지 완료했고 공개 운영본은 버전 107이다. 버전 108~139는 버전 140으로 대체해 배포하지 않는다. 버전 140 공개 배포에는 정확히 `버전 140 운영 배포 승인`이라는 사용자 명시 승인이 필요하다. 30분 간격 자동 개발은 활성 상태로 다음 고우선순위 감사를 이어간다. 다음 감사는 상담 FLOW 첨부 다운로드가 저장된 파일 크기와 R2 실제 객체 크기를 응답 전후에 대조해 손상되거나 교체된 첨부 본문을 제공하지 않는지 확인한다. 본문 해시·스키마를 새로 도입하거나 기존 첨부를 자동 수정·삭제하지 않는다. 관리자 안정 ID가 실제로 바뀌는 경우의 결속 초기화는 계정 소유권 확인·운영 D1 백업·감사가 필요한 수동 복구이며 자동화하지 않는다. 실제 파트너 계정·고객 데이터·외부 발송·유료 AI·보관 및 삭제 정책은 별도 승인 없이 사용하거나 변경하지 않는다.
