...
- 최신 기능 커밋: `49886b3cd7107ce8998b2f24150957bdda7723dc` (`fix: recheck FLOW access after upload reservation`)
- 최신 확인: 기존 부분 R2 재시도의 예약 조회 중 사건·담당 계정 삭제, 예약 없는 첫 multipart 요청의 예약 생성 직후 업로드 권한 회수
- 연속 확인: 수정 전 두 경쟁 모두 상태 변경 뒤 R2 쓰기 2회 재현, 예약 직후 접근 재검사로 수정 후 새 R2·FLOW D1 쓰기 0건, 삭제·결속 폐기 또는 권한 회수와 복구 가능한 `pending` 예약 보존, 복구 뒤 같은 예약으로 한 번 완료
- 검증: Node 회귀 검사 752개, 격리 workerd/D1/R2 검사 522개, 타입검사, 전체 lint, 변경 파일 포맷, 프로덕션 빌드와 로컬 Worker 화면·인증 경계 통과
...
- 현재 공개 운영본: 버전 107
- 최신 Sites 저장 버전: 299 (`appgprj_6a92514801988191b79eb9bd314e3fcd~appgver_59328952b68c81919629e801b2be8474`), 소스 `49886b3cd7107ce8998b2f24150957bdda7723dc`
- 운영 상태: 서버 오류 로그 개인정보 보완본 버전 107이 공개 운영 중이다. 버전 108–298은 더 완전한 후보인 버전 299로 대체한다.
...
먼저 [상담 FLOW 업로드 예약 중 접근변경 경쟁 무결성](FLOW_UPLOAD_RESERVATION_ACCESS_RACE_2026_09_07.md)을 확인한다.
...
Sites 버전 299 저장·검증까지 완료했고 공개 운영본은 버전 107이다. 버전 108–298은 버전 299로 대체해 배포하지 않는다. 공개 교체는 정확한 `버전 299 운영 배포 승인`이 필요하다. 30분 간격 자동 개발은 활성 상태로 첫 R2 객체 저장 직후 두 번째 객체 쓰기 전에 권한·사건 상태가 바뀔 때, 중간 접근 재검사가 두 번째 R2 쓰기와 FLOW D1을 차단하고 첫 객체·두 예약을 복구 가능하게 보존하는지 감사한다. 실제 파트너 계정·고객 데이터·외부 발송·유료 AI는 별도 승인 없이 사용하지 않는다.
