...
- 최신 기능 커밋: `e03d8e35c8f9fce28f42374a2bc0e30013752095` (`fix: reject unsafe AI diagnosis text`)
- 최신 기능: 신규 Step 0 실행 신원·완료 결과의 손상 UTF-16, 제어문자와 `U+FFFD` 저장 차단
- 검증: Node 회귀 검사 642개, 격리 workerd/D1/R2 검사 449개, 타입검사, 전체 lint, 변경 파일 포맷 검사, 프로덕션 빌드와 로컬 운영 Worker 화면 3곳 HTTP 200·CSP·`DENY`·`nosniff`·`no-referrer` 통과
...
- 현재 공개 운영본: 버전 107
- 최신 Sites 저장 버전: 187 (`appgprj_6a92514801988191b79eb9bd314e3fcd~appgver_bb26e479b3e8819197f6e65bb929a275`), 소스 `e03d8e35c8f9fce28f42374a2bc0e30013752095`
- 운영 상태: 서버 오류 로그 개인정보 보완본 버전 107이 공개 운영 중이다. 버전 108~186은 버전 187로 대체해 배포하지 않으며, AI 진단 문자열 무결성 보완본 버전 187이 정확한 버전 운영 배포 승인 대기 중이다.
...
먼저 [AI 진단 문자열 무결성 경계](AI_DIAGNOSIS_TEXT_INTEGRITY_2026_09_05.md)를 확인한다.
...
Sites 버전 187 저장·검증까지 완료했고 공개 운영본은 버전 107이다. 버전 108~186은 버전 187로 대체해 배포하지 않는다. 버전 187 공개 배포에는 정확히 `버전 187 운영 배포 승인`이라는 사용자 명시 승인이 필요하다. 30분 간격 자동 개발은 활성 상태로 다음 고우선순위 감사를 이어간다. 다음 감사는 AI 실행 원장의 시작·완료 시각과 공급자 토큰 사용량이 애플리케이션과 D1에서 같은 의미·범위를 유지하는지 확인한다. 관리자 안정 ID가 실제로 바뀌는 경우의 결속 초기화는 계정 소유권 확인·운영 D1 백업·감사가 필요한 수동 복구이며 자동화하지 않는다. 실제 파트너 계정·고객 데이터·외부 발송·유료 AI·보관 및 삭제 정책은 별도 승인 없이 사용하거나 변경하지 않는다.
