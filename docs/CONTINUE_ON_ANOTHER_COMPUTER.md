...
- 최신 기능 커밋: `7ef10a7079d7d09d47530caae2fc3ddb42ca96a4` (`fix: resume FLOW uploads across devices`)
- 최신 기능: 탭 종료·다른 기기의 새 요청 ID도 같은 상담·행위자·명령/파일 지문인 미완료 FLOW 첨부 예약만 재사용하고 실제 성공 명령 ID를 불변 완료 원장에 결속
- 검증: Node 회귀 검사 711개, 격리 workerd/D1/R2 검사 521개, 타입검사, 전체 lint, 변경 파일 포맷, 프로덕션 빌드와 로컬 Worker 화면·인증 경계 통과
...
- 현재 공개 운영본: 버전 107
- 최신 Sites 저장 버전: 256 (`appgprj_6a92514801988191b79eb9bd314e3fcd~appgver_83d3269982d08191a08b1fe299a43cfe`), 소스 `7ef10a7079d7d09d47530caae2fc3ddb42ca96a4`
- 운영 상태: 서버 오류 로그 개인정보 보완본 버전 107이 공개 운영 중이다. Sites 안전 게이트가 정확한 공개 대상 버전 승인을 요구하며, 버전 108–255는 버전 256으로 대체해 배포하지 않는다.
...
먼저 [상담 FLOW 교차 탭·기기 첨부 재시도 무결성](FLOW_UPLOAD_CROSS_DEVICE_RESUME_INTEGRITY_2026_09_06.md)을 확인한다.
...
Sites 버전 256 저장·검증까지 완료했고 공개 운영본은 버전 107이다. 버전 108–255는 버전 256으로 대체해 배포하지 않는다. 공개 교체는 정확한 `버전 256 운영 배포 승인`이 필요하다. 30분 간격 자동 개발은 활성 상태로 파일·음성 2슬롯 교차 기기 재시도와 슬롯 불일치 거절 경계 감사를 이어간다. 실제 파트너 계정·고객 데이터·외부 발송·유료 AI는 별도 승인 없이 사용하지 않는다.
