...
- 최신 기능 커밋: `ab7a71ea7d969b5b45bc5f615fb6bec08a35a76b` (`fix: retain AI provider message evidence`)
- 최신 기능: Anthropic 성공 Message의 `type`·`role`·고유 ID 검증과 Step 0 완료 증거 보존
...
- 현재 공개 운영본: 버전 107
- 최신 Sites 저장 버전: 193 (`appgprj_6a92514801988191b79eb9bd314e3fcd~appgver_de3ace62450881919d0503c58243264b`), 소스 `ab7a71ea7d969b5b45bc5f615fb6bec08a35a76b`
- 운영 상태: 서버 오류 로그 개인정보 보완본 버전 107이 공개 운영 중이다. 버전 108–192는 버전 193으로 대체해 배포하지 않으며, AI 공급자 Message 신원 보완본 버전 193이 정확한 버전 운영 배포 승인 대기 중이다.
...
먼저 [AI 공급자 Message 신원 무결성 경계](AI_PROVIDER_MESSAGE_IDENTITY_INTEGRITY_2026_09_06.md)를 확인한다.
...
Sites 버전 193 저장·검증까지 완료했고 공개 운영본은 버전 107이다. 버전 108–192는 버전 193으로 대체해 배포하지 않는다. 버전 193 공개 배포에는 정확히 `버전 193 운영 배포 승인`이라는 사용자 명시 승인이 필요하다. 30분 간격 자동 개발은 활성 상태로 다음 고우선순위 감사를 이어간다. 다음 감사는 성공 응답의 예상하지 않은 비텍스트 content block 처리를 확인한다. 관리자 안정 ID가 실제로 바뀌는 경우의 결속 초기화는 계정 소유권 확인·운영 D1 백업·감사가 필요한 수동 복구이며 자동화하지 않는다. 실제 파트너 계정·고객 데이터·외부 발송·유료 AI·보관 및 삭제 정책은 별도 승인 없이 사용하거나 변경하지 않는다.
