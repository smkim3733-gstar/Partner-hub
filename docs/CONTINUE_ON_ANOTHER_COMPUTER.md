...
- 최신 기능 커밋: `50de3de2f256c78425e9bfc777133cb9be1ff4a8` (`fix: bound AI diagnosis run fields`)
- 최신 기능: 신규 Step 0 실행의 요청 ID 형식과 진행·기업·지침·모델·실행자 상한
- 검증: Node 회귀 검사 642개, 격리 workerd/D1/R2 검사 448개, 타입검사, 전체 lint, 변경 파일 포맷 검사, 프로덕션 빌드와 로컬 운영 Worker 화면 3곳 HTTP 200·CSP·`DENY`·`nosniff`·`no-referrer` 통과
...
- 현재 공개 운영본: 버전 107
- 최신 Sites 저장 버전: 186 (`appgprj_6a92514801988191b79eb9bd314e3fcd~appgver_7d6d61fb778c8191b628294f00a542ad`), 소스 `50de3de2f256c78425e9bfc777133cb9be1ff4a8`
- 운영 상태: 서버 오류 로그 개인정보 보완본 버전 107이 공개 운영 중이다. 버전 108~185는 버전 186으로 대체해 배포하지 않으며, AI 진단 실행 신원 필드 보완본 버전 186이 정확한 버전 운영 배포 승인 대기 중이다.
...
Sites 버전 186 저장·검증까지 완료했고 공개 운영본은 버전 107이다. 버전 108~185는 버전 186으로 대체해 배포하지 않는다. 버전 186 공개 배포에는 정확히 `버전 186 운영 배포 승인`이라는 사용자 명시 승인이 필요하다. 30분 간격 자동 개발은 활성 상태로 다음 고우선순위 감사를 이어간다. 다음 감사는 AI 실행 신원과 완료 결과 문자열이 제어문자·짝 없는 UTF-16 surrogate를 저장하지 않도록 애플리케이션과 SQLite 경계를 통일한다. 관리자 안정 ID가 실제로 바뀌는 경우의 결속 초기화는 계정 소유권 확인·운영 D1 백업·감사가 필요한 수동 복구이며 자동화하지 않는다. 실제 파트너 계정·고객 데이터·외부 발송·유료 AI·보관 및 삭제 정책은 별도 승인 없이 사용하거나 변경하지 않는다.
