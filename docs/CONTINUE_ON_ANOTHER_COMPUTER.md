...
- 최신 기능 커밋: `24f134b1f23fa245378aa0929d38910f318ae99b` (`fix: stabilize FLOW admin command actor`)
- 최신 기능: 상담 FLOW 관리자 명령의 단일 논리 행위자 신원 결속
- 검증: Node 회귀 검사 661개, 격리 workerd/D1/R2 검사 481개, 타입검사, 전체 lint, 변경 파일 포맷 검사, 프로덕션 빌드와 로컬 운영 Worker 화면 3곳 HTTP 200·CSP·`DENY`·`nosniff`·`no-referrer` 통과
...
- 현재 공개 운영본: 버전 107
- 최신 Sites 저장 버전: 215 (`appgprj_6a92514801988191b79eb9bd314e3fcd~appgver_c8163449155c8191be00ad10e9cdf7a7`), 소스 `24f134b1f23fa245378aa0929d38910f318ae99b`
- 운영 상태: 서버 오류 로그 개인정보 보완본 버전 107이 공개 운영 중이다. 버전 108–214는 버전 215로 대체해 배포하지 않는다.
...
먼저 [상담 FLOW 관리자 명령 행위자 신원 무결성](FLOW_ADMIN_COMMAND_ACTOR_INTEGRITY_2026_09_06.md)을 확인한다.
...
Sites 버전 215 저장·검증까지 완료했고 공개 운영본은 버전 107이다. 버전 108–214는 버전 215로 대체해 배포하지 않는다. 30분 간격 자동 개발은 활성 상태로 다음 고우선순위 감사를 이어간다. 다음 감사는 관리자 명령 행위자 표시의 대표 역할 결속과 명령 동작·FLOW 상태 전이 의미 일치를 확인한다. 관리자 안정 ID가 실제로 바뀌는 경우의 결속 초기화는 계정 소유권 확인·운영 D1 백업·감사가 필요한 수동 복구이며 자동화하지 않는다. 실제 파트너 계정·고객 데이터·외부 발송·유료 AI·보관 및 삭제 정책은 별도 승인 없이 사용하거나 변경하지 않는다.
