...
- 최신 로컬 기능 커밋: `ae2e3ab` (`fix: bind flow receipt semantics in inventory`)
- 최신 확인: 완료 FLOW payload 명령 영수증의 표시 행위자 `actor` 또는 동작 `action`이 동일 명령 감사기록과 어긋난 손상 상태
- 연속 확인: 두 상태 모두 `inconsistent`·`unavailable` 격리, 증명 제외, R2 `head` 전 503 차단과 영수증 복구 후 SHA-256 증명 복귀; 두 의미 필드가 모두 없는 레거시 영수증 유지
- 검증: Node 회귀 검사 780개, 관련 회귀 54개, 격리 workerd/D1/R2 검사 589개, 추가형 마이그레이션 98개 2회, 타입검사, 전체 lint, 변경 파일 포맷, 프로덕션 빌드와 로컬 Worker 화면·인증 경계 통과
- GitHub: `https://github.com/smkim3733-gstar/Partner-hub`, 원격 `main`은 `ee40a91b5de4ef8fed06245ba2b636e623773b0c`; 로컬 버전 328 푸시는 앱 자동 보안 검토가 이번 변경의 명시 승인을 요구해 보류
...
- 운영 상태: 서버 오류 로그 개인정보 보완본 버전 107이 공개 운영 중이다. 버전 108–327은 더 완전한 로컬 후보인 버전 328로 대체한다.
...
먼저 [파일 재고 FLOW 영수증 의미 결속](FILE_INVENTORY_FLOW_RECEIPT_SEMANTIC_BINDING_2026_09_08.md)을 확인한다.
...
Sites 최신 저장 버전은 309이고 공개 운영본은 버전 107이다. 버전 108–327은 버전 328로 대체해 배포하지 않는다. GitHub 반영에는 정확히 `버전 328 GitHub main 푸시 승인`, 버전 328의 Sites 소스 전송·버전 저장·공개 교체에는 정확히 `버전 328 Sites 소스 전송 및 공개 운영 배포 승인`이 필요하다. 30분 간격 자동 개발은 활성 상태로 FLOW payload 명령 영수증과 감사기록의 `action`을 함께 같은 거짓값으로 바꿔도 실제 업로드 예약의 `slot`·`purpose` 및 완료 명령 효과와 어긋난 상태를 파일 재고가 정상 증명으로 오인하지 않는지 다음 순서로 감사한다. 실제 파트너 계정·고객 데이터·외부 발송·유료 AI는 별도 승인 없이 사용하지 않는다.
