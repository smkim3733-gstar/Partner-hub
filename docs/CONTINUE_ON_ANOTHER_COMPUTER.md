...
- 최신 기능 커밋: `a4fef5ab9fa6550465827559a5671d7206f91bc5` (`fix: make file integrity ledgers append-only`)
- 최신 기능: 기업자료 R2 저장 키·객체 무결성 원장의 직접 수정·선행 삭제를 D1에서 차단하고 세 불변 원장의 운영 코드 직접 변경을 정적 차단
- 검증: Node 회귀 검사 620개, 격리 workerd/D1/R2 검사 433개, 타입검사, 전체 lint, 변경 파일 포맷 검사, 프로덕션 빌드와 로컬 운영 Worker 화면 3곳 HTTP 200·CSP·`DENY`·`nosniff`·`no-referrer` 통과
...
- 현재 공개 운영본: 버전 107
- 최신 Sites 저장 버전: 171 (`appgprj_6a92514801988191b79eb9bd314e3fcd~appgver_54375a0472948191b14443782191de6a`), 소스 `a4fef5ab9fa6550465827559a5671d7206f91bc5`
- 운영 상태: 서버 오류 로그 개인정보 보완본 버전 107이 공개 운영 중이다. 버전 108~170은 버전 171로 대체해 배포하지 않으며, 기업자료 저장 키·객체 무결성 원장 불변 보완본 버전 171이 정확한 버전 운영 배포 승인 대기 중이다.
...
먼저 [기업자료 저장 키·객체 무결성 원장 불변 경계](COMPANY_FILE_INTEGRITY_LEDGER_IMMUTABILITY_2026_09_05.md)를 확인한다.
...
Sites 버전 171 저장·검증까지 완료했고 공개 운영본은 버전 107이다. 버전 108~170은 버전 171로 대체해 배포하지 않는다. 버전 171 공개 배포에는 정확히 `버전 171 운영 배포 승인`이라는 사용자 명시 승인이 필요하다. 30분 간격 자동 개발은 활성 상태로 다음 고우선순위 감사를 이어간다. 다음 감사는 기업자료 담당 계정·진행 연결 원장의 정상 변경 필요성과 최초 결속 뒤 불변 범위를 구분하고 저장소 경계를 보완한다. 관리자 안정 ID가 실제로 바뀌는 경우의 결속 초기화는 계정 소유권 확인·운영 D1 백업·감사가 필요한 수동 복구이며 자동화하지 않는다. 실제 파트너 계정·고객 데이터·외부 발송·유료 AI·보관 및 삭제 정책은 별도 승인 없이 사용하거나 변경하지 않는다.
