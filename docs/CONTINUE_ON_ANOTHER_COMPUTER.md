...
- 최신 기능 커밋: `1faa1fe769c1060974df73f92dd1b06de3391e34` (`fix: enforce portal state payload capacity`)
- 최신 기능: 전체 운영 상태의 D1 payload를 900,000 UTF-8 바이트 이하로 제한
- 검증: Node 회귀 검사 633개, 격리 workerd/D1/R2 검사 443개, 타입검사, 전체 lint, 변경 파일 포맷 검사, 프로덕션 빌드와 로컬 운영 Worker 화면 3곳 HTTP 200·CSP·`DENY`·`nosniff`·`no-referrer` 통과
...
- 현재 공개 운영본: 버전 107
- 최신 Sites 저장 버전: 181 (`appgprj_6a92514801988191b79eb9bd314e3fcd~appgver_f35a099752608191b59c48e1368a70e1`), 소스 `1faa1fe769c1060974df73f92dd1b06de3391e34`
- 운영 상태: 서버 오류 로그 개인정보 보완본 버전 107이 공개 운영 중이다. 버전 108~180은 버전 181로 대체해 배포하지 않으며, 포털 상태 payload 용량 보완본 버전 181이 정확한 버전 운영 배포 승인 대기 중이다.
...
Sites 버전 181 저장·검증까지 완료했고 공개 운영본은 버전 107이다. 버전 108~180은 버전 181로 대체해 배포하지 않는다. 버전 181 공개 배포에는 정확히 `버전 181 운영 배포 승인`이라는 사용자 명시 승인이 필요하다. 30분 간격 자동 개발은 활성 상태로 다음 고우선순위 감사를 이어간다. 다음 감사는 로그인 통계 행의 계정 ID·누적 횟수·마지막 로그인 시각이 허용된 전이로만 저장되는지 D1 수명주기 경계와 모든 쓰기 경로를 점검한다. 관리자 안정 ID가 실제로 바뀌는 경우의 결속 초기화는 계정 소유권 확인·운영 D1 백업·감사가 필요한 수동 복구이며 자동화하지 않는다. 실제 파트너 계정·고객 데이터·외부 발송·유료 AI·보관 및 삭제 정책은 별도 승인 없이 사용하거나 변경하지 않는다.
