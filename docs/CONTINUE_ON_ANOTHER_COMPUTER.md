...
- 최신 기능 커밋: `f2aa49b722bb03af14042ae7ee186f5390f936b5` (`fix: enforce FLOW AI evidence transitions`)
- 최신 기능: 상담 FLOW AI 증거·이력·작업·감사기록의 불변 전이와 새 작업의 `queued/blocked` 무증거 시작을 앱·D1에서 함께 강제
- 검증: Node 회귀 검사 649개, 격리 workerd/D1/R2 검사 458개, 타입검사, 전체 lint, 변경 파일 포맷 검사, 프로덕션 빌드와 로컬 운영 Worker 화면 3곳 HTTP 200·CSP·`DENY`·`nosniff`·`no-referrer` 통과
...
- 현재 공개 운영본: 버전 107
- 최신 Sites 저장 버전: 202 (`appgprj_6a92514801988191b79eb9bd314e3fcd~appgver_0f5969a7927c8191a0235624f8c1d37d`), 소스 `f2aa49b722bb03af14042ae7ee186f5390f936b5`
- 운영 상태: 서버 오류 로그 개인정보 보완본 버전 107이 공개 운영 중이다. 버전 108–201은 버전 202로 대체해 배포하지 않으며, FLOW AI 증거·감사 D1 전이 무결성본 버전 202가 정확한 버전 운영 배포 승인 대기 중이다.
...
먼저 [상담 FLOW AI 증거·감사 D1 전이 무결성](FLOW_AI_EVIDENCE_D1_TRANSITION_INTEGRITY_2026_09_06.md)을 확인한다.
...
Sites 버전 202 저장·검증까지 완료했고 공개 운영본은 버전 107이다. 버전 108–201은 버전 202로 대체해 배포하지 않는다. 버전 202 공개 배포에는 정확히 `버전 202 운영 배포 승인`이라는 사용자 명시 승인이 필요하다. 30분 간격 자동 개발은 활성 상태로 다음 고우선순위 감사를 이어간다. 다음 감사는 D1 신규 FLOW 삽입에서 완료·실패 작업과 공급자 증거를 조작해 넣는 경로를 확인한다. 관리자 안정 ID가 실제로 바뀌는 경우의 결속 초기화는 계정 소유권 확인·운영 D1 백업·감사가 필요한 수동 복구이며 자동화하지 않는다. 실제 파트너 계정·고객 데이터·외부 발송·유료 AI·보관 및 삭제 정책은 별도 승인 없이 사용하거나 변경하지 않는다.
