...
- 최신 감사 커밋: `51f325c48895a9c6d5a0c408e6b61bf1b20fd21b` (`test: reject changed FLOW retry bytes`)
- 최신 확인: 부분 R2 저장 뒤 같은 명령 ID에 이름·형식·크기는 같고 바이트만 다른 첨부를 넣으면 R2 쓰기 전 409로 차단하고 기존 객체·예약 보존
...
- 현재 공개 운영본: 버전 107
- 최신 Sites 저장 버전: 265 (`appgprj_6a92514801988191b79eb9bd314e3fcd~appgver_33058a369b8c8191bda770aad53d456e`), 소스 `51f325c48895a9c6d5a0c408e6b61bf1b20fd21b`
- 운영 상태: 서버 오류 로그 개인정보 보완본 버전 107이 공개 운영 중이다. 버전 108–264는 더 완전한 후보인 버전 265로 대체한다.
...
먼저 [상담 FLOW 재시도 첨부 불일치 무결성](FLOW_RETRY_ATTACHMENT_MISMATCH_INTEGRITY_2026_09_07.md)을 확인한다.
...
Sites 버전 265 저장·검증까지 완료했고 공개 운영본은 버전 107이다. 버전 108–264는 버전 265로 대체해 배포하지 않는다. 공개 교체는 정확한 `버전 265 운영 배포 승인`이 필요하다. 30분 간격 자동 개발은 활성 상태로 부분 R2 실패 뒤 같은 명령 ID의 명령 본문 변경 재시도가 예약 객체를 건드리지 못하는지 감사한다. 실제 파트너 계정·고객 데이터·외부 발송·유료 AI는 별도 승인 없이 사용하지 않는다.
