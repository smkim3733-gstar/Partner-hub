...
- 최신 기능 커밋: `fa03089b786f52abc89d98a7438344290bff7833` (`fix: validate FLOW stored file formats`)
- 최신 기능: FLOW 저장 확장자·고정 MIME·목적 조합의 공용 등록표 검증과 SQLite 축소 전 손상 형식 격리
- 검증: Node 회귀 검사 591개, 격리 workerd/D1/R2 검사 390개, 타입검사, 전체 lint, 변경 파일 포맷 검사, 프로덕션 빌드와 로컬 운영 Worker 화면 3곳 HTTP 200·CSP·`DENY`·`nosniff`·`no-referrer` 통과
...
- 현재 공개 운영본: 버전 107
- 최신 Sites 저장 버전: 156 (`appgprj_6a92514801988191b79eb9bd314e3fcd~appgver_b06702e5b7888191be653ee687a7e597`), 소스 `fa03089b786f52abc89d98a7438344290bff7833`
- 운영 상태: 서버 오류 로그 개인정보 보완본 버전 107이 공개 운영 중이다. 버전 108~155는 버전 156으로 대체해 배포하지 않으며, FLOW 파일 확장자·MIME·목적 보완본 버전 156이 정확한 버전 운영 배포 승인 대기 중이다.
...
먼저 [상담 FLOW 파일 확장자·MIME·목적 무결성 경계](FLOW_FILE_FORMAT_INTEGRITY_2026_09_05.md)를 확인한다.
...
Sites 버전 156 저장·검증까지 완료했고 공개 운영본은 버전 107이다. 버전 108~155는 버전 156으로 대체해 배포하지 않는다. 버전 156 공개 배포에는 정확히 `버전 156 운영 배포 승인`이라는 사용자 명시 승인이 필요하다. 30분 간격 자동 개발은 활성 상태로 다음 고우선순위 감사를 이어간다. 다음 감사는 FLOW 저장 파일의 R2 키가 `consulting-flow/{파일 ID}`에 정확히 결속되는지 확인하고 상세·다운로드·SQLite 대시보드 경계를 일치시킨다. 관리자 안정 ID가 실제로 바뀌는 경우의 결속 초기화는 계정 소유권 확인·운영 D1 백업·감사가 필요한 수동 복구이며 자동화하지 않는다. 실제 파트너 계정·고객 데이터·외부 발송·유료 AI·보관 및 삭제 정책은 별도 승인 없이 사용하거나 변경하지 않는다.
