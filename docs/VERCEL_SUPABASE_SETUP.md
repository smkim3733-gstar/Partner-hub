...
같은 날 기존 Supabase Secret key를 Git 제외 로컬 설정에 저장하고 실제 비공개 버킷 조회 HTTP 200을 확인했습니다. OAuth 인증 후 새 연결에서 MCP 도구 11개도 조회됐습니다. 이는 DB 접속·관리자 설정·데이터 이관·업무 사용 완료 증거는 아닙니다. DB 비밀번호를 포함한 연결 URI는 아직 입력이 필요합니다.
