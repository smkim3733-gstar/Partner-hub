...
import { applicationAttachmentCategoryProblem, companyCategoryLabel, companyFileProblem, companyPortalFileAccept, applicationAttachmentTitle, MAX_APPLICATION_FILES, MAX_COMPANY_FILE_MEGABYTES, type ApplicationAttachment } from '@/lib/company-file-policy';
import {
...
function isPortalState(value: unknown): value is PortalState {
  ...
  const operationalRecordsValid = operationalRecordStateError(
    state.tasks,
          ...
          {visibleDocuments.length ? <div className="grid gap-4 lg:grid-cols-2">
            {visibleDocuments.map((document) => {
              ...
              return <article key={document.id} className="rounded-2xl border border-slate-200 bg-white p-5">
                <div className="flex items-start justify-between gap-3"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><Pill tone="navy">{companyCategoryLabel(document.category)}</Pill><Pill tone={statusTone}>{document.status}</Pill>{document.sensitive ? <Pill tone="slate"><LockKeyhole className="mr-1 size-3" aria-hidden="true" />민감자료</Pill> : null}{document.storageFileId ? <Pill tone="green">보안저장 완료</Pill> : null}</div><p className="mt-3 text-xs font-semibold text-slate-500">{document.company}</p>{document.caseId && <p className="mt-1 text-[11px] text-slate-400" title={document.caseId}>연결 진행 {document.caseId.slice(-8)}</p>}<h2 className="mt-1 text-base font-bold text-slate-950">{document.title}</h2>{document.fileName ? <p className="mt-2 [overflow-wrap:anywhere] text-xs leading-5 text-slate-500">{document.fileName}{document.fileSize ? ` · ${readableFileSize(document.fileSize)}` : ''}</p> : <p className="mt-2 text-xs leading-5 text-amber-700">아직 제출된 파일이 없습니다.</p>}{document.storageFileId ? <CompanyFileDownload fileId={document.storageFileId} fileName={document.fileName || document.title} className="mt-3 inline-flex min-h-10 items-center gap-2 rounded-xl border border-sky-100 bg-sky-50 px-3 text-xs font-bold text-[#075f93] hover:bg-sky-100"><LockKeyhole className="size-3.5" aria-hidden="true" /> 권한 확인 후 원본 내려받기</CompanyFileDownload> : null}</div><span className="grid size-11 shrink-0 place-items-center rounded-xl bg-slate-50 text-[#0877b8]"><FileText className="size-5" aria-hidden="true" /></span></div>
      ...
      {uploadOpen ? <PortalDialog titleId="upload-modal-title" onClose={closeUploadDialog} closeDisabled={uploading}>
        <div className="w-full max-w-2xl overflow-hidden rounded-2xl bg-white shadow-2xl">
          ...
          <div className="grid max-h-[65vh] gap-5 overflow-y-auto p-5 md:grid-cols-2">
            ...
            <div className="md:col-span-2"><label className="flex min-h-32 cursor-pointer flex-col items-center justify-center rounded-2xl border-2 border-dashed border-slate-200 bg-slate-50 px-4 text-center hover:border-sky-300 hover:bg-sky-50"><Upload className="size-7 text-[#0877b8]" aria-hidden="true" /><span className="mt-3 text-sm font-semibold text-slate-800">{uploadFile?.name || 'PDF·이미지·엑셀·워드·녹취 파일 선택'}</span><span className="mt-1 text-xs text-slate-500">파일당 {MAX_COMPANY_FILE_MEGABYTES}MB 이하 · MP3, M4A, WAV 녹취 포함</span><input type="file" accept={companyPortalFileAccept} className="sr-only" onChange={(event) => { setUploadFile(event.target.files?.[0] ?? null); setUploadConsent(false); setUploadError(''); }} /></label></div>
