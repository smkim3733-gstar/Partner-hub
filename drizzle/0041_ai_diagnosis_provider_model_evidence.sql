...
WHEN NEW.status = '대표 검토 대기'
  AND NOT COALESCE((
    ...
    AND (SELECT COUNT(*) FROM json_each(NEW.result_json)) = 11
    AND NOT EXISTS (
      ...
      WHERE root.key NOT IN (
        ...
        'companyOverview', 'confirmedStrengths', 'mainRisks',
        'solutionCandidates', 'verificationQuestions', 'missingDocuments',
        'complianceNotes', 'nextAction'
      )
    )
