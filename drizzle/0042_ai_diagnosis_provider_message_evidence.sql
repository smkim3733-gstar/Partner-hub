...
WHEN NEW.status = '대표 검토 대기'
  AND NOT COALESCE((
    ...
    AND (SELECT COUNT(*) FROM json_each(NEW.result_json)) = 12
    AND NOT EXISTS (
      SELECT 1 FROM json_each(NEW.result_json) AS root
      WHERE root.key NOT IN (
        '_requestFingerprint', '_providerRequestId', '_providerModel',
        '_providerMessageId', 'companyOverview', 'confirmedStrengths',
        'mainRisks', 'solutionCandidates', 'verificationQuestions',
        'missingDocuments', 'complianceNotes', 'nextAction'
      )
    )
    ...
    AND json_type(NEW.result_json, '$._providerModel') = 'text'
    AND trim(json_extract(NEW.result_json, '$._providerModel')) <> ''
    AND json_extract(NEW.result_json, '$._providerModel') = trim(json_extract(NEW.result_json, '$._providerModel'))
    AND length(json_extract(NEW.result_json, '$._providerModel')) <= 200
    AND json_type(NEW.result_json, '$._providerMessageId') = 'text'
    AND trim(json_extract(NEW.result_json, '$._providerMessageId')) <> ''
    AND json_extract(NEW.result_json, '$._providerMessageId') = trim(json_extract(NEW.result_json, '$._providerMessageId'))
    AND length(json_extract(NEW.result_json, '$._providerMessageId')) <= 512
