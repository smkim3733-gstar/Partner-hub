...
void test('completed FLOW reservation metadata and completion command drift stay inconsistent and fail before R2 access', async () => {
  const metadataCaseId = 'flow-receipt-metadata-drift-case';
  const metadataFileId = 'flow-receipt-metadata-drift';
  const metadataCommandId = 'flow-receipt-metadata-command';
  const commandCaseId = 'flow-receipt-command-drift-case';
  const commandFileId = 'flow-receipt-command-drift';
  const commandId = 'flow-receipt-command';
  ...
  await seed(
    ...
    [
      {
        id: metadataCaseId,
        company: 'FLOW 예약 메타데이터 손상기업',
        trainee: member.name,
        partnerMemberId: member.id,
      },
      {
        id: commandCaseId,
        company: 'FLOW 완료 명령 손상기업',
        trainee: member.name,
        partnerMemberId: member.id,
      },
    ],
  ...
  const db = await flowDatabase();
  for (const [caseId, receiptCommandId] of [
    [metadataCaseId, metadataCommandId],
    [commandCaseId, commandId],
  ] as const) {
    const row = await db
      .prepare('SELECT payload FROM consulting_flows WHERE case_id = ?1')
      .bind(caseId)
    ...
    flow.commandIds = [receiptCommandId];
    flow.commandReceipts = {
      [receiptCommandId]: { actorKey, fingerprint },
    };
    await mutateConsultingFlowFixture(
      db,
  ...
  assert.equal(
    beforeDrift.items.filter((item) =>
      [metadataFileId, commandFileId].includes(item.id),
    ).length,
    2,
  );
  await db.batch([
  ...
  try {
    await db.batch([
      db
        .prepare(`INSERT INTO consulting_flow_upload_requests
          (case_id, actor_key, command_id, slot, fingerprint, file_id,
            ...
            created_at, status)
          VALUES (?1, ?2, ?3, 'file', ?4, ?5, ?6, 'drifted-name.txt',
            'text/plain', 4, 'report', ?7, 'ready')`)
        .bind(
          metadataCaseId,
          actorKey,
          metadataCommandId,
          fingerprint,
          metadataFileId,
          `consulting-flow/${metadataFileId}`,
          date,
        ),
      db
        .prepare(`INSERT INTO consulting_flow_upload_requests
      ...
      db
        ...
        .bind(metadataFileId, metadataCommandId),
      db
        .prepare(`INSERT INTO consulting_flow_upload_completions
          (file_id, command_id) VALUES (?1, 'different-completion-command')`)
        .bind(commandFileId),
    ]);
  } finally {
    await db.batch([
  ...
  try {
    ...
    assert.equal(
      inconsistent.integrityCoverage.metadata,
      beforeDrift.integrityCoverage.metadata - 2,
    );
    assert.equal(
      inconsistent.integrityCoverage.unavailable,
      beforeDrift.integrityCoverage.unavailable + 2,
    );
    const driftItems = inconsistent.items.filter(
      (item) =>
        item.source === 'flow' &&
        [metadataFileId, commandFileId].includes(item.id),
    );
    ...
    assert.doesNotMatch(
      ...
      /drifted-name|different-completion-command|actor_key|fingerprint|consulting-flow\//,
    );
    for (const fileId of [metadataFileId, commandFileId]) {
      const response = await presence(request(), {
        params: Promise.resolve({ id: fileId }),
      });
      assert.equal(response.status, 503, await response.clone().text());
    ...
    try {
      await db.batch([
        ...[metadataFileId, commandFileId].map((fileId) =>
          db
            .prepare(
              'DELETE FROM consulting_flow_upload_completions WHERE file_id = ?1',
            )
...
void test('completed FLOW receipt actor and fingerprint drift stay inconsistent and fail before R2 access', async () => {
  const actorCaseId = 'flow-receipt-actor-drift-case';
  const actorFileId = 'flow-receipt-actor-drift';
  const actorCommandId = 'flow-receipt-actor-command';
  const fingerprintCaseId = 'flow-receipt-fingerprint-drift-case';
  const fingerprintFileId = 'flow-receipt-fingerprint-drift';
  const fingerprintCommandId = 'flow-receipt-fingerprint-command';
  const actorKey = FLOW_ADMIN_COMMAND_ACTOR_KEY;
  ...
  await seed(
    ...
    [
      {
        id: actorCaseId,
        company: 'FLOW 영수증 행위자 손상기업',
        trainee: member.name,
        partnerMemberId: member.id,
      },
      {
        id: fingerprintCaseId,
        company: 'FLOW 영수증 지문 손상기업',
        trainee: member.name,
        partnerMemberId: member.id,
      },
    ],
  );
  await completedFlowFile(actorFileId, actorCaseId);
  await completedFlowFile(fingerprintFileId, fingerprintCaseId);
  const db = await flowDatabase();
  for (const [caseId, commandId] of [
    [actorCaseId, actorCommandId],
    [fingerprintCaseId, fingerprintCommandId],
  ] as const) {
    const row = await db
      .prepare('SELECT payload FROM consulting_flows WHERE case_id = ?1')
      .bind(caseId)
  ...
  try {
    await db.batch(
      [
        [actorCaseId, actorCommandId, actorFileId],
        [fingerprintCaseId, fingerprintCommandId, fingerprintFileId],
      ].flatMap(([caseId, commandId, fileId]) => [
        db
          .prepare(`INSERT INTO consulting_flow_upload_requests
            (case_id, actor_key, command_id, slot, fingerprint, file_id,
              storage_key, original_name, content_type, size_bytes, purpose,
  ...
  assert.equal(
    beforeDrift.items.filter((item) =>
      [actorFileId, fingerprintFileId].includes(item.id),
    ).length,
    2,
  );
  for (const [caseId, commandId, field, value] of [
    [actorCaseId, actorCommandId, 'actorKey', 'member:drifted-receipt-actor'],
    [fingerprintCaseId, fingerprintCommandId, 'fingerprint', 'e'.repeat(64)],
  ] as const) {
    const row = await db
      .prepare('SELECT payload FROM consulting_flows WHERE case_id = ?1')
      .bind(caseId)
  ...
  try {
    ...
    assert.doesNotMatch(
      JSON.stringify(driftItems),
      /flow-receipt-(actor|fingerprint)-command|admin:primary|drifted-receipt-actor|e{64}|"actorKey"|"fingerprint"|actor_key|consulting-flow\//,
    );
    for (const fileId of [actorFileId, fingerprintFileId]) {
      const response = await presence(request(), {
        params: Promise.resolve({ id: fileId }),
      });
      assert.equal(response.status, 503, await response.clone().text());
    ...
    try {
      await db.batch([
        ...[actorFileId, fingerprintFileId].map((fileId) =>
          db
            .prepare(
              'DELETE FROM consulting_flow_upload_completions WHERE file_id = ?1',
            )
...
void test('completed FLOW report upload whose saved report loses the receipt file stays inconsistent before R2 access', async () => {
  const caseId = 'flow-report-receipt-effect-drift-case';
  const fileId = 'flow-report-receipt-effect-drift';
  const commandId = 'flow-report-receipt-effect-command';
  const reportId = `${commandId}-report`;
  const actorKey = FLOW_ADMIN_COMMAND_ACTOR_KEY;
  ...
  await completedFlowFile(fileId, caseId);
  const db = await flowDatabase();
  const setReportFileId = async (value?: string) => {
    const row = await db
      .prepare('SELECT payload FROM consulting_flows WHERE case_id = ?1')
      .bind(caseId)
      .first<{ payload: string }>();
    ...
    const report = flow.reports.find(
      (candidate: { id?: unknown }) => candidate.id === reportId,
    );
    assert.ok(report);
  ...
  flow.reports = [
    {
      ...
      version: 1,
      title: '1차 정밀진단보고서',
      body: '보고서 업로드 영수증과 저장된 보고서 첨부 연결을 검증하는 가상 본문입니다. 외부 자료나 실제 기업정보를 사용하지 않습니다. '.repeat(
        2,
      ),
      fileId,
      createdAt: date,
  ...
  flow.audit = [
    {
      ...
      actor,
      action: 'save_report',
      detail: '1차 정밀진단보고서 V1 저장 · 담당 파트너 공유',
    },
  ];
  await mutateConsultingFlowFixture(
    db,
  ...
  try {
    ...
    assert.doesNotMatch(
      JSON.stringify(item),
      /flow-report-receipt-effect-command|actor_key|fingerprint|consulting-flow\//,
    );
    const response = await presence(request(), {
      params: Promise.resolve({ id: fileId }),
    });
...
void test('completed FLOW transcript upload whose receipt or recording target drifts stays inconsistent before R2 access', async () => {
  const caseId = 'flow-transcript-target-drift-case';
  const fileId = 'flow-transcript-target-drift';
  const commandId = 'flow-transcript-target-command';
  const recordingId = 'flow-transcript-target-recording';
  const actorKey = FLOW_ADMIN_COMMAND_ACTOR_KEY;
  const actor = FLOW_ADMIN_COMMAND_ACTOR_NAME;
  ...
  await seed(
    ...
    [
      {
        id: caseId,
        company: 'FLOW 전사문 영수증 대상 손상기업',
        trainee: member.name,
        partnerMemberId: member.id,
      },
    ],
  );
  await completedFlowFile(fileId, caseId, undefined, caseId, 'transcript');
  const db = await flowDatabase();
  const row = await db
    .prepare('SELECT payload FROM consulting_flows WHERE case_id = ?1')
    .bind(caseId)
  ...
  assert.ok(row);
  const flow = JSON.parse(row.payload);
  const meetingId = 'flow-transcript-target-meeting';
  flow.meetings = [
    {
      id: meetingId,
      kind: 'first',
  ...
  flow.recordings = [
    {
      ...
      transcript:
        'FLOW 전사문 영수증과 실제 녹취 대상 파일 연결을 검증하는 가상 전사문입니다.',
      transcriptReviewedAt: date,
      transcriptReviewedBy: actorKey,
      consentAt: date,
      createdAt: date,
  ...
  const mutateTarget = async (
    ...
    if (kind === 'receipt') {
      ...
      const recording = current.recordings.find(
        (candidate: { id?: unknown }) => candidate.id === recordingId,
      );
      assert.ok(recording);
      ...
      db
        .prepare(`INSERT INTO consulting_flow_upload_requests
          (case_id, actor_key, command_id, slot, fingerprint, file_id,
            ...
            created_at, status)
          VALUES (?1, ?2, ?3, 'file', ?4, ?5, ?6, ?7,
            'text/plain', 4, 'transcript', ?8, 'ready')`)
        .bind(
          caseId,
          actorKey,
          commandId,
  ...
  const drifts = [
    {
      ...
      corrupt: () => mutateTarget('recording'),
      restore: () => mutateTarget('recording', fileId),
    },
    {
      label: 'receipt recording',
      corrupt: () => mutateTarget('receipt', 'forged-recording-target'),
      restore: () => mutateTarget('receipt', recordingId),
    },
  ];
  try {
    for (const drift of drifts) {
      ...
      assert.doesNotMatch(
        JSON.stringify(item),
        /flow-transcript-target-command|flow-transcript-target-recording|actor_key|fingerprint|consulting-flow\//,
      );
      const response = await presence(request(), {
        params: Promise.resolve({ id: fileId }),
      });
    ...
    bucket.head = originalHead;
    await mutateTarget('receipt', recordingId);
    await mutateTarget('recording', fileId);
    await db.batch([
      db.prepare(
        'DROP TRIGGER IF EXISTS consulting_flow_upload_completions_no_delete',
      ),
...
void test('completed FLOW requested document whose receipt or request file target drifts stays inconsistent before R2 access', async () => {
  const caseId = 'flow-requested-document-target-drift-case';
  const fileId = 'flow-requested-document-target-drift';
  const commandId = 'flow-requested-document-target-command';
  const requestId = 'flow-requested-document-target-request';
  ...
  const mutateTarget = async (kind: 'receipt' | 'request', value?: string) => {
    const stored = await db
      .prepare('SELECT payload FROM consulting_flows WHERE case_id = ?1')
      .bind(caseId)
      .first<{ payload: string }>();
    ...
    } else if (value === undefined) delete current.requests[0].fileId;
    else current.requests[0].fileId = value;
    await mutateConsultingFlowFixture(
      db,
      'UPDATE consulting_flows SET payload = ?1 WHERE case_id = ?2',
      [JSON.stringify(current), caseId],
      ...
      db
        .prepare(`INSERT INTO consulting_flow_upload_requests
          (case_id, actor_key, command_id, slot, fingerprint, file_id,
            ...
            created_at, status)
          VALUES (?1, ?2, ?3, 'file', ?4, ?5, ?6, ?7,
            'application/pdf', 4, 'requested_document', ?8, 'ready')`)
        .bind(
          caseId,
          actorKey,
          commandId,
  ...
  const drifts = [
    {
      label: 'request file',
      corrupt: () => mutateTarget('request'),
      restore: () => mutateTarget('request', fileId),
    },
    {
      label: 'receipt request',
      corrupt: () => mutateTarget('receipt', 'forged-request-target'),
      restore: () => mutateTarget('receipt', requestId),
    },
  ];
  try {
    for (const drift of drifts) {
      ...
      assert.doesNotMatch(
        JSON.stringify(item),
        /flow-requested-document-target-(?:command|request)|actor_key|fingerprint|consulting-flow\//,
      );
      const response = await presence(request(), {
        params: Promise.resolve({ id: fileId }),
      });
