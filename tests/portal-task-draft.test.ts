...
void test('internal work uses its fixed non-company scope', () => {
  assert.deepEqual(preparePortalTaskDraft({
    title: '월간 운영회의 준비',
    company: '',
    due: '금요일',
...
void test('task draft refuses each blank field instead of storing sample or fallback text', () => {
  assert.deepEqual(preparePortalTaskDraft({ title: ' ', company: '회사', due: '오늘', dueState: 'today', kind: '서류요청' }), {
    ok: false,
    error: '업무명을 입력해 주세요.',
  });
  assert.deepEqual(preparePortalTaskDraft({ title: '업무', company: ' ', due: '오늘', dueState: 'today', kind: '서류요청' }), {
    ok: false,
    error: '기업명을 입력해 주세요.',
  });
  assert.deepEqual(preparePortalTaskDraft({ title: '업무', company: '회사', due: ' ', dueState: 'today', kind: '서류요청' }), {
    ok: false,
    error: '마감일을 입력해 주세요.',
  });
  assert.deepEqual(preparePortalTaskDraft({ title: '업무', company: '회사', due: '오늘', dueState: '', kind: '서류요청' }), {
    ok: false,
    error: '마감 구분을 선택해 주세요.',
  });
});

void test('task draft bounds every user-written display field', () => {
  const base = { title: '업무', company: '회사', due: '오늘', dueState: 'today', kind: '서류요청' };
