...
void test('second dual-slot R2 write failure preserves the first object and resumes both reservations', async () => {
  ...
  const transcript = new File(
    ['SYNTHETIC_PARTIAL_WRITE_TRANSCRIPT'],
    'partial-write-transcript.txt',
    { type: 'text/plain' },
  );
  const audio = new File(
    [new Uint8Array([0x49, 0x44, 0x33, 4, 0, 2])],
    'partial-write-audio.mp3',
    { type: 'audio/mpeg' },
  );
  const firstCommandId = `dual-slot-partial-write-${++sequence}`;
  const previousKeys = new Set(objects.keys());
  const bucket = flowBucket();
  const put = bucket.put.bind(bucket);
  let putAttempts = 0;
  ...
  assert.deepEqual(
    [...objects.keys()].filter((key) => !previousKeys.has(key)),
    [pending.find(({ slot }) => slot === 'file')!.storage_key],
  );

  const retryCommandId = `dual-slot-partial-write-retry-${++sequence}`;
  const retry = await POST(
    request(
      stored.caseId,
      command,
...
void test('same command ID retry resumes both reservations after the second R2 write fails', async () => {
  const stored = await transcriptJobFixture(false, body);
  const command = {
    type: 'save_recording',
    meetingId: stored.meetings.at(-1)!.id,
    transcript: `${body} 동일 명령 2슬롯 재시도`,
    transcriptReviewed: true,
    recordingConsent: true,
    privacyMasked: true,
    fileConsent: true,
  ...
  const transcript = new File(
    ['SYNTHETIC_SAME_COMMAND_RETRY_TRANSCRIPT'],
    'same-command-retry-transcript.txt',
    { type: 'text/plain' },
  );
  const audio = new File(
    [new Uint8Array([0x49, 0x44, 0x33, 4, 0, 5])],
    'same-command-retry-audio.mp3',
    { type: 'audio/mpeg' },
  );
  const commandId = `dual-slot-same-command-retry-${++sequence}`;
  const previousKeys = new Set(objects.keys());
  const bucket = flowBucket();
  const put = bucket.put.bind(bucket);
  let putAttempts = 0;
...
void test('stale revision retry after a partial R2 write preserves reservations until an exact refreshed retry', async () => {
  const stored = await transcriptJobFixture(false, body);
  const command = {
    type: 'save_recording',
    meetingId: stored.meetings.at(-1)!.id,
    transcript: `${body} 낡은 revision 재시도 차단`,
    transcriptReviewed: true,
    recordingConsent: true,
    privacyMasked: true,
    fileConsent: true,
  ...
  const transcript = new File(
    ['SYNTHETIC_STALE_REVISION_TRANSCRIPT'],
    'stale-revision-transcript.txt',
    { type: 'text/plain' },
  );
  const audio = new File(
    [new Uint8Array([0x49, 0x44, 0x33, 4, 0, 7])],
    'stale-revision-audio.mp3',
    { type: 'audio/mpeg' },
  );
  const commandId = `dual-slot-stale-revision-${++sequence}`;
  const previousKeys = new Set(objects.keys());
  const bucket = flowBucket();
  const put = bucket.put.bind(bucket);
  let putAttempts = 0;
  ...
  const transcriptReservation = pending.find(({ slot }) => slot === 'file')!;
  const storedBeforeConflict = new Uint8Array(
    objects.get(transcriptReservation.storage_key),
  ).slice();
  assert.deepEqual(
    [...objects.keys()].filter((key) => !previousKeys.has(key)),
  ...
  const concurrentCommandId = `dual-slot-concurrent-policy-${++sequence}`;
  ...
  assert.equal(concurrent.status, 200, await concurrent.clone().text());
  const concurrentFlow = (await readFlow(stored.caseId))!;
  assert.equal(concurrentFlow.revision, stored.revision + 1);
  assert.equal(concurrentFlow.ai.enabled, false);

  let stalePutAttempts = 0;
  bucket.put = async (...args: Parameters<R2Bucket['put']>) => {
    stalePutAttempts++;
    return put(...args);
  };
  ...
  assert.deepEqual(await stale.json(), {
    error: '다른 변경이 있습니다. 새로고침 후 내용을 확인해 주세요.',
  });
  assert.equal(stalePutAttempts, 0);
  assert.deepEqual(
    new Uint8Array(objects.get(transcriptReservation.storage_key)),
    storedBeforeConflict,
  );
  assert.deepEqual(await readFlow(stored.caseId), concurrentFlow);
  assert.deepEqual(
    (
      await db
        .prepare(
  ...
  assert.equal(recording.transcriptFileId, reservedBySlot.get('file'));
  assert.equal(recording.audioFileId, reservedBySlot.get('audio'));
  assert.deepEqual(saved.commandIds.slice(-2), [
    concurrentCommandId,
    commandId,
  ]);
  assert.deepEqual(
...
void test('partner retry rechecks account and permissions before resuming partial R2 writes', async () => {
  ...
  for (const scenario of scenarios) {
    ...
    const command = {
      type: 'save_recording',
      meetingId: stored.meetings.at(-1)!.id,
      transcript: `${body} 파트너 재권한 ${scenario.kind}`,
      transcriptReviewed: true,
      recordingConsent: true,
      privacyMasked: true,
      fileConsent: true,
    ...
    const transcript = new File(
      [`SYNTHETIC_PARTNER_REAUTH_TRANSCRIPT_${scenario.kind}`],
      `partner-reauth-${scenario.kind}.txt`,
      { type: 'text/plain' },
    );
    const audio = new File(
      [new Uint8Array([0x49, 0x44, 0x33, 4, 0, 8])],
      `partner-reauth-${scenario.kind}.mp3`,
      { type: 'audio/mpeg' },
    );
    const commandId = `dual-slot-partner-reauth-${scenario.kind}-${++sequence}`;
    const previousKeys = new Set(objects.keys());
    const bucket = flowBucket();
    const put = bucket.put.bind(bucket);
    let putAttempts = 0;
...
void test('partner retry rechecks assignment and lifecycle before resuming partial R2 writes', async () => {
  const scenarios = [
    {
      kind: 'reassigned-case',
      status: 409,
      error:
        '전체 진행현황의 담당 계정과 상담 FLOW 담당 계정이 일치하지 않습니다. 대표가 담당 정보를 확인해 주세요.',
    },
    {
      kind: 'discontinued-case',
      status: 409,
      error:
  ...
  for (const scenario of scenarios) {
    ...
    const command = {
      type: 'save_recording',
      meetingId: stored.meetings.at(-1)!.id,
      transcript: `${body} 담당·진행 재확인 ${scenario.kind}`,
      transcriptReviewed: true,
      recordingConsent: true,
      privacyMasked: true,
      fileConsent: true,
    ...
    const transcript = new File(
      [`SYNTHETIC_PARTNER_CASE_RECHECK_TRANSCRIPT_${scenario.kind}`],
      `partner-case-recheck-${scenario.kind}.txt`,
      { type: 'text/plain' },
    );
    const audio = new File(
      [new Uint8Array([0x49, 0x44, 0x33, 4, 0, 9])],
      `partner-case-recheck-${scenario.kind}.mp3`,
      { type: 'audio/mpeg' },
    );
    const commandId = `dual-slot-partner-case-recheck-${scenario.kind}-${++sequence}`;
    const previousKeys = new Set(objects.keys());
    const bucket = flowBucket();
    const put = bucket.put.bind(bucket);
    let putAttempts = 0;
    ...
    const originalState = (await readPortalState()) as {
      cases: Array<{
        id: string;
        trainee: string;
    ...
    if (scenario.kind === 'reassigned-case') {
      changedCase.trainee = '가상 다른 담당자';
      changedCase.partnerMemberId = 'other';
    } else {
      changedCase.pipelineLifecycleVersion = 1;
      changedCase.pipelineLifecycleStatus = 'discontinued';
    }
    ...
    await writePortalState(originalState);
    const retry = await POST(
      request(
        stored.caseId,
        command,
...
void test('partner retry rechecks case and account existence before resuming partial R2 writes', async () => {
  const scenarios = [
    {
      kind: 'deleted-case',
      status: 404,
  ...
  for (const scenario of scenarios) {
    ...
    const command = {
      type: 'save_recording',
      meetingId: stored.meetings.at(-1)!.id,
      transcript: `${body} 사건·계정 존재 재확인 ${scenario.kind}`,
      transcriptReviewed: true,
      recordingConsent: true,
      privacyMasked: true,
      fileConsent: true,
    ...
    const transcript = new File(
      [`SYNTHETIC_PARTNER_EXISTENCE_RECHECK_TRANSCRIPT_${scenario.kind}`],
      `partner-existence-recheck-${scenario.kind}.txt`,
      { type: 'text/plain' },
    );
    const audio = new File(
      [new Uint8Array([0x49, 0x44, 0x33, 4, 0, 10])],
      `partner-existence-recheck-${scenario.kind}.mp3`,
      { type: 'audio/mpeg' },
    );
    const commandId = `dual-slot-partner-existence-recheck-${scenario.kind}-${++sequence}`;
    const previousKeys = new Set(objects.keys());
    const bucket = flowBucket();
    const put = bucket.put.bind(bucket);
    let putAttempts = 0;
    ...
    if (scenario.kind === 'deleted-partner-and-case')
      changedState.members = changedState.members.filter(
        ({ id }) => id !== partner.id,
      );
    await writePortalState(changedState);
...
async function assertPartialR2RetryHandlesMultipartStateChange(
  ...
  const changeState = async () => {
    ...
    if (scenario.kind === 'permission') {
      ...
      changedCase.partnerMemberId = 'other';
    } else if (scenario.kind === 'case-deletion') {
...
void test('partial R2 retry rejects login email changes during the first object write and recovers after identity restoration', async () => {
  await assertPartialR2RetryHandlesMultipartStateChange({
    kind: 'email-change',
    timing: 'first-r2-write',
    status: 403,
