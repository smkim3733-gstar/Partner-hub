...
const run = {
  ...
  providerMessageId: 'msg_synthetic_response',
  result: {
...
void test('mismatched or malformed Step 0 results never reach the screen', async () => {
  for (const value of [
    ...
    { ...run, usage: { inputTokens: 0, outputTokens: 20 } },
    { ...run, usage: { inputTokens: 10, outputTokens: 0 } },
    { ...run, usage: { inputTokens: 10, outputTokens: 4_001 } },
    ...
    { ...run, providerMessageId: 'm'.repeat(513) },
