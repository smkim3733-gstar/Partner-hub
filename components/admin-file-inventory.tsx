...
function presenceIntegrityLabel(presence: InventoryPresence) {
  if (presence.integrityMatches === false)
    return ' · 현재 객체 정보와 저장 원장 불일치: 사용 중지·복구 필요';
  if (!presence.integrityMatches) return '';
  if (presence.integrityProof === 'sha256')
    return ' · 현재 객체 정보가 저장 시 SHA-256·ETag·MIME 원장과 일치 · 본문 미읽음';
  if (presence.integrityProof === 'etag')
    return ' · 현재 객체 정보가 레거시 ETag·MIME 원장과 일치 · 본문 미읽음';
  return ' · 현재 객체 정보가 레거시 크기·MIME 원장과 일치 · 본문 미읽음';
}
        ...
        {!opened ? (
          ...
          <>
            ...
            <p className="rounded-lg bg-sky-50 p-3 text-sm leading-6 text-sky-950">
              ...
              연결 확인 필요는 삭제 가능 판정이 아닙니다. 목록과 적용 현황은
              데이터베이스의 저장 원장 기준입니다. 현재 원본 확인은 파일 본문을
              읽지 않고 존재·크기·고정 MIME·ETag·저장 시 SHA-256 객체 정보만
              원장과 대조합니다.
            </p>
            {busy && (
              ...
              <>
                ...
                >
                  ...
                  <p className="mt-2 text-xs leading-5 text-muted-foreground">
                    현재 저장소 객체를 검사한 결과가 아닙니다. 파일 본문을
                    읽거나 레거시 체크섬을 추정하지 않습니다.
                  </p>
                </div>
                {!page.items.length ? (
                          ...
                          {presence && (
                            <output className="block text-xs leading-5">
                              {typeof presence === 'string'
                                ? presence
                                : `${presence.exists ? '현재 원본 존재' : '현재 원본 없음'} · ${sizeLabel(presence.sizeBytes)}${presence.sizeMatches === false ? ' · 저장 기록과 크기 불일치: 사용 중지·복구 필요' : ''}${presenceIntegrityLabel(presence)} · ${dateLabel(presence.checkedAt)}`}
                            </output>
                          )}
