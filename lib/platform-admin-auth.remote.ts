import 'server-only';
import { standaloneRequestConfig as nextBackendConfig } from './next-backend-config.server';
...
function configuredRequest(request: Request) {
  if (!nextBackendRequestAllowed(request, nextBackendConfig()))
    throw new PasswordError('설정된 보안 사이트에서 다시 요청해 주세요.', 403);
}
