...
export type DocumentRequestDueState = 'overdue' | 'today' | 'upcoming';
...
function koreanDateKey(now: Date) {
...
export function documentRequestDueState(
  dueDate: string,
  now = new Date(),
): DocumentRequestDueState | null {
...
export function prepareDocumentRequest(
  ...
  if (!dueState) return { ok: false as const, field: 'dueDate' as const, error: '올바른 제출기한을 선택해 주세요.' };
  ...
  return {
    ...
    dueDate,
    dueState,
    skippedOutstanding: normalized.size - available.length,
  };
}
